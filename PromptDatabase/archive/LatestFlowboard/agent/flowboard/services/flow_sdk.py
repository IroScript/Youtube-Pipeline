...
def _media_get_url(media_id: str) -> str:
    ...
    NOTE (2026-08): Google Flow API stopped accepting dot-notation query
    parameters (``clientContext.tool=PINHOLE``) on the ``/v1/media`` GET
    endpoint — requests now return ``400 INVALID_ARGUMENT``. Removed the
    query string; the tool context is already embedded in the dispatch
    body's ``clientContext`` block and is not required for polling.
    """
    return f"{FLOW_API_BASE}/v1/media/{media_id}"
...
class FlowSDK:
    ...
    async def check_async(
        ...
        NOTE (2026-08): Google Flow API deprecated the ``/v1/media/<id>``
        GET endpoint for status polling (returns ``INVALID_ARGUMENT``).
        We now route ALL operation names — including workflow-sourced ones
        — through ``batchCheckAsyncVideoGenerationStatus`` which remains
        the reliable polling path. The ``_poll_workflows`` fallback is
        kept for edge cases where ``batchCheckAsync`` doesn't cover an op.
        """
        ops_summary: list[dict[str, Any]] = []
        raw_old: Any = None
        resolved_names: set[str] = set()

        # Try batchCheckAsync for ALL operation names first — this is the
        # canonical polling path that the Flow web UI itself uses.
        if operation_names:
            body = {
                "operations": [
            ...
            if isinstance(raw_old, dict) and raw_old.get("error"):
                # batchCheckAsync itself failed — fall through to workflow polling
                logger.warning("batchCheckAsync failed: %s", raw_old.get("error"))
            else:
                batch_ops = extract_video_operations(raw_old, requested=operation_names)
                ops_summary.extend(batch_ops)
                resolved_names = {op.get("name") for op in batch_ops if isinstance(op, dict) and op.get("name")}

        # Fallback: for any ops NOT resolved by batchCheckAsync, try
        # the workflow media-poll path (kept for backward compat).
        raw_workflows: list[dict[str, Any]] = []
        if workflows:
            unresolved_wfs = [w for w in workflows if isinstance(w, dict) and w.get("name") not in resolved_names]
            if unresolved_wfs:
                wf_summary, raw_workflows = await self._poll_workflows(unresolved_wfs)
