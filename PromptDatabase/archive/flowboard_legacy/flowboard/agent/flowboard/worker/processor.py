...
VIDEO_POLL_MAX_CYCLES = 90  # 3x extended timeout: 90 cycles × 10s = 900s (15 min)
...
async def _handle_gen_video(params: dict) -> tuple[dict, Optional[str]]:
    ...
    while (
        ...
        if job_id:
            from flowboard.routes.automate import _update_job_status
            elapsed_render = int(poll_attempts * VIDEO_POLL_INTERVAL_S)
            pct = min(95, 40 + int((poll_attempts / VIDEO_POLL_MAX_CYCLES) * 55))
            _update_job_status(
                job_id,
                progress_pct=pct,
                stage="generating_video",
                stage_message=f"Rendering 8s Veo 3.1 Lite video... ({elapsed_render}s elapsed)",
            )
