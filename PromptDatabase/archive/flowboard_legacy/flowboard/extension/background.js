...
function sendToAgent(msg) {
  if (msg.id) {
    fetch(CALLBACK_URL, {
      ...
      .then((res) => {
        if (!res.ok) {
          console.warn(`[Flowboard] HTTP callback returned ${res.status}, falling back to WS`);
          if (ws?.readyState === WebSocket.OPEN) ws.send(JSON.stringify(msg));
        }
      })
      .catch((err) => {
        console.warn('[Flowboard] HTTP callback fetch failed, falling back to WS:', err);
        if (ws?.readyState === WebSocket.OPEN) ws.send(JSON.stringify(msg));
      });
    return;
  }
...
async function handleApiRequest(msg) {
  ...
  try {
    ...
    // Human-like pre-action pause (3s to 8s) to prevent bot / security detection
    await sleep(3000 + Math.random() * 5000);

    const fetchHeaders = { ...(headers || {}), authorization: `Bearer ${flowKey}` };

    const controller = new AbortController();
    const fetchTimeout = setTimeout(() => controller.abort(), 450000); // 3x extended timeout: 450s (7.5 min)
    let response;
    try {
    ...
    const results = await chrome.scripting.executeScript({
      ...
      func: async (action, siteKey) => {
        ...
        while (!window.grecaptcha?.enterprise?.execute) {
          if (Date.now() - start > 15000) throw new Error('grecaptcha not available');
          await new Promise((r) => setTimeout(r, 200));
        }
        return await window.grecaptcha.enterprise.execute(siteKey, { action });
      },
      args: [pageAction, '6LdsFiUsAAAAAIjVDZcuLhaHiDn5nnHVXVRQGeMV'],
    });
    if (results && results[0] && results[0].result) {
      return { token: results[0].result };
    }
  } catch (err) {
    console.warn('[Flowboard] Direct MAIN world captcha execution fallback:', err);
  }
...
async function solveCaptcha(requestId, captchaAction) {
  ...
  // Try candidate Flow tabs — retry up to 10 attempts per candidate with human-like delays
    ...
    for (let attempt = 1; attempt <= 10; attempt++) {
      ...
      try {
        const resp = await Promise.race([
          ...
          new Promise((_, rej) => setTimeout(() => rej(new Error('CAPTCHA_TIMEOUT')), 90000)),
        ]);
        if (resp?.token) return resp;
        const errMsg = resp?.error || 'CAPTCHA_NO_TOKEN';
        errors.push(errMsg);
        // If captcha fails, reload tab and retry with randomized human delay (3s - 10s)
        if (attempt < 10 && (errMsg.includes('grecaptcha') || errMsg.includes('TIMEOUT') || errMsg.includes('CONTENT_TIMEOUT'))) {
          console.warn(`[Flowboard] Captcha attempt ${attempt}/10 failed (${errMsg}), reloading tab ${live.id} and retrying...`);
          try { await chrome.tabs.reload(live.id); } catch {}
        ...
        if (attempt < 10 && (msg.includes('grecaptcha') || msg.includes('TIMEOUT'))) {
          await sleep(3000 + Math.random() * 5000);
          continue;
        }
        return { error: msg };
