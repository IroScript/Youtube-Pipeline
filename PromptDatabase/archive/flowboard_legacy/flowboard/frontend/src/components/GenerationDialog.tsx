...
export function GenerationDialog() {
  ...
  async function handleSubmit() {
    ...
    if (isVideo) {
      ...
      //
      // 2026-07-10: cap the final prompt + camera at 480 chars to stay
      // under Flow's ~500-char textInput ceiling. Flow rejects longer
      // payloads with INVALID_ARGUMENT ("Request contains an invalid
      // argument"). If the user's prompt is already long we drop the
      // camera suffix entirely (the trade-off is acceptable; the model
      // still produces motion without the explicit constraint).
      const camInstruction = cameraInstruction(camera);
      const cameraSuffix = camInstruction ? `. ${camInstruction}` : '';
      const MAX_PROMPT_LEN = 4800;
      let videoPrompt: string;
      if (finalPrompt.length + cameraSuffix.length <= MAX_PROMPT_LEN) {
        videoPrompt = finalPrompt + cameraSuffix;
      } else if (finalPrompt.length <= MAX_PROMPT_LEN) {
        // Drop the camera suffix to avoid the INVALID_ARGUMENT path.
        videoPrompt = finalPrompt;
      } else {
        videoPrompt = finalPrompt.slice(0, MAX_PROMPT_LEN);
      }
        ...
        {!isCharacter && (
          <div className="gen-dialog__field">
            <div className="gen-dialog__label-row">
              ...
              <span className="gen-dialog__char-count">{prompt.length}/5000</span>
            </div>
            <textarea
              id="gen-prompt"
              ref={firstFocusRef}
