...
def _media_get_url(media_id: str) -> str:
    ...
    2026-07-10: dropped ``?clientContext.tool=PINHOLE`` query string.
    Flow rejected it with INVALID_ARGUMENT — query params don't translate
    to the request body's clientContext field. The plain path is what
    /v1/media/<id> accepts and returns the encoded MP4 once ready.
    """
...
class FlowSDK:
    ...
    async def _poll_workflows(
        ...
        for wf in workflows:
            ...
            # Transport / API failure — keep polling. Treat 404 AND 400 as
            # "not ready" / transient. Flow returns 400 INVALID_ARGUMENT on
            # /v1/media/<id> while the video is still queued in the
            # scheduler (before its primary mediaId has been activated) and
            # only flips to 200 with encoded MP4 bytes once rendering
            # completes. Marking `done: True` on 400 was the bug — it
            # collapsed a 30-90s render into an instant failure. Keep
            # polling until we either get bytes OR the worker hits its
            # max-cycle timeout. Real terminal errors come back as 4xx
            # with a content-filter reason in the body (e.g.
            # PUBLIC_ERROR_PROMINENT_PEOPLE_FILTER_FAILED) which we'll
            # pick up once the media is "live" on Flow's side.
            if not isinstance(resp, dict):
                ops_summary.append(
            ...
            if isinstance(status_code, int) and status_code >= 400 and status_code not in (400, 404):
