"""
V2 Sample Generator — McCullen Nanomites Dialogue
===================================================
সব AI থেকে v2 অডিও তৈরি।

ব্যবহার: python generate_v2_all.py <PORT>
"""
import paramiko
import os
import sys
...
def run_and_download(client, sftp, name, script_content, remote_dir, local_dir):
    print(f"\n{'='*60}")
    print(f"  {name} — v2 McCullen Dialogue")
    print(f"{'='*60}")
    
    remote_script = f"/content/omnivoice/gen_{name}_v2.py"
...
def main():
    ...
    run_and_download(client, sftp, "edge_tts",
        get_script(os.path.join(LOCAL_BASE, "edge_tts", "v2_edge_tts.py")),
        "/content/omnivoice/edge_tts", os.path.join(LOCAL_BASE, "edge_tts"))
    
    run_and_download(client, sftp, "bark_ai",
        get_script(os.path.join(LOCAL_BASE, "bark_ai", "v2_bark_ai.py")),
    ...
    if "ok" in health:
        run_and_download(client, sftp, "omnivoice",
            get_script(os.path.join(LOCAL_BASE, "omnivoice", "v2_omnivoice.py")),
