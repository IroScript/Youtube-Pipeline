"""
V5 Ultra Human Voice Generator
================================
OmniVoice shimmer voice + emotions + quality boost

ব্যবহার: python generate_v5_ultra_human_voice_all.py <PORT>
"""
...
def main():
    ...
    script = get_script(os.path.join(LOCAL_BASE, "omnivoice", "v5_ultra_human_voice_omnivoice.py"))
    with sftp.open("/content/omnivoice/gen_omnivoice_v5.py", "w") as f:
        f.write(script)
    
    _, o, e = client.exec_command(ENV + "cd /content/omnivoice && python3 gen_omnivoice_v5.py", timeout=600)
    ...
    client.close()
    print(f"\n{'='*60}")
    print("  V5 ULTRA HUMAN VOICE COMPLETE!")
    print(f"{'='*60}")
