...
export class DOMQueryEngine {
  ...
  static async simulateClickElement(el, label = 'element') {
    ...
    // Realistic human delay between 3 to 7 seconds after every button click
    const waitSec = (3.0 + Math.random() * 4.0).toFixed(1);
    await new Promise(r => setTimeout(r, parseFloat(waitSec) * 1000));
    return true;
  }
