...
const pendingNativeDownloadNames = []; // page-triggered upscale downloads
const pendingExtensionDownloadNames = []; // chrome.downloads.download() names
const PENDING_NATIVE_DOWNLOAD_NAMES_STORAGE_KEY = 'flow_pending_native_download_names';
// Content scripts are injected programmatically and older anonymous listeners
// can remain alive until the Flow tab itself is reloaded. Version every command
// that can create/rename/download media so stale listeners simply ignore it.
...
const NATIVE_DOWNLOAD_NAME_TTL_MS = 10 * 60 * 1000;
const EXTENSION_DOWNLOAD_NAME_TTL_MS = 60 * 1000;
// Flow upscale itself may take the full three-minute UI timeout and emit the
// native download slightly later. Keep a guarded UUID/media match alive long
// enough for slow 2K/4K jobs while staying below the 10-minute reservation TTL.
const NATIVE_DOWNLOAD_MATCH_WINDOW_MS = 8 * 60 * 1000;
const pendingNativeDownloadNamesReady = chrome.storage.session
    .get(PENDING_NATIVE_DOWNLOAD_NAMES_STORAGE_KEY)
    .then((data) => {
        const now = Date.now();
        const saved = Array.isArray(data?.[PENDING_NATIVE_DOWNLOAD_NAMES_STORAGE_KEY])
            ? data[PENDING_NATIVE_DOWNLOAD_NAMES_STORAGE_KEY]
            : [];
        pendingNativeDownloadNames.splice(0, pendingNativeDownloadNames.length, ...saved
            .filter((entry) => entry?.relativeBase && Number(entry.expiresAt) > now)
            .map((entry) => ({
                token: String(entry.token || ''),
                relativeBase: String(entry.relativeBase || ''),
                createdAt: Number(entry.createdAt) || Math.max(0, Number(entry.expiresAt) - NATIVE_DOWNLOAD_NAME_TTL_MS),
    ...
    .catch((error) => {
        console.warn('[Downloads] Could not restore pending file names:', error);
    });
...
function rememberCompletedDownloadRequest(requestKey = '') {
    if (!requestKey) return;
    sessionSeenImages.add(requestKey);
    if (sessionSeenImages.size <= SESSION_SEEN_MAX) return;
    const entries = Array.from(sessionSeenImages);
    entries.slice(0, SESSION_SEEN_TRIM).forEach((key) => sessionSeenImages.delete(key));
}

function runDownloadRequestOnce(requestKey, operation) {
    const existing = activeDownloadRequests.get(requestKey);
    if (existing) return existing;

    const pending = Promise.resolve().then(operation);
    activeDownloadRequests.set(requestKey, pending);
    pending.finally(() => {
        if (activeDownloadRequests.get(requestKey) === pending) {
            activeDownloadRequests.delete(requestKey);
        }
    }).catch(() => {});
    return pending;
}
...
const ACCOUNT_DISABLED_MESSAGES = Object.freeze({
    en: 'Abusive activity has been detected. All linked accounts, including your primary account, have been blocked and can no longer use Google Flow Automator on the Starter plan.',
    ko: '비정상적인 이용 활동이 감지되었습니다. 기본 계정을 포함해 이 브라우저에 연결된 모든 계정이 차단되었으며, 더 이상 Starter 플랜으로 Google Flow Automator를 사용할 수 없습니다.',
    ja: '不正利用が検出されました。メインアカウントを含む、関連付けられたすべてのアカウントがブロックされ、StarterプランではGoogle Flow Automatorを利用できなくなりました。',
    zh: '检测到滥用行为。包括主账号在内的所有关联账号均已被封禁，无法再通过Starter方案使用Google Flow Automator。',
    de: 'Missbräuchliche Nutzung wurde erkannt. Alle verknüpften Konten, einschließlich Ihres Hauptkontos, wurden gesperrt und können Google Flow Automator nicht mehr im Starter-Tarif verwenden.',
    fr: 'Une utilisation abusive a été détectée. Tous les comptes associés, y compris votre compte principal, ont été bloqués et ne peuvent plus utiliser Google Flow Automator avec l’offre Starter.',
    hi: 'दुरुपयोग गतिविधि का पता चला है। आपके मुख्य खाते सहित सभी जुड़े खातों को ब्लॉक कर दिया गया है और अब वे Starter प्लान पर Google Flow Automator का उपयोग नहीं कर सकते।'
});
const ACCOUNT_DISABLED_MESSAGE = ACCOUNT_DISABLED_MESSAGES.en;
function getLocalizedAccountDisabledMessage(settingsOrLanguage = {}) {
    const language = typeof settingsOrLanguage === 'string'
        ? settingsOrLanguage
        : settingsOrLanguage?.uiLanguage;
    return ACCOUNT_DISABLED_MESSAGES[language] || ACCOUNT_DISABLED_MESSAGE;
}
async function loadLocalizedAccountDisabledMessage() {
    const settings = await storage.getSettings().catch(() => null);
    return getLocalizedAccountDisabledMessage(settings);
}
...
function getStarterQuotaReachedMessage() {
    return `Starter daily quota reached. You can run up to ${STARTER_ACCOUNT_PROMPT_LIMIT} prompts per day.`;
}
...
async function isPremiumImageDownloaderUnlocked() {
    ...
    if (gate.disabled || !hasValidAuth || !gateUidMatches) return false;
...
async function isBulkDownloadBaseNameUnlocked() {
    // Bulk filename prefixes share the Premium+ membership boundary today,
    // but keep a separate trust-boundary function so asset entitlement changes
    // cannot accidentally expose filename bulk-setting later.
...
async function assertCanRunPromptByMembership() {
    ...
    if (membership.disabled) {
        const error = await loadLocalizedAccountDisabledMessage();
        return { allowed: false, error, membership };
    }
...
async function blockImageDownloaderAction(actionName = 'Image Downloader') {
    const [data, settings] = await Promise.all([
        chrome.storage.local.get(FIRESTORE_GATE_CACHE_KEY),
        storage.getSettings().catch(() => null)
    ]);
    const gate = data[FIRESTORE_GATE_CACHE_KEY] || {};
    const isDisabled = gate.disabled === true;
    const error = isDisabled
        ? getLocalizedAccountDisabledMessage(settings)
        : 'Premium Feature - Unlock Required for Image Downloader.';
    ...
    try {
        ...
        await Promise.all(flowTabs.map(tab => new Promise((resolve) => {
            chrome.tabs.sendMessage(tab.id, {
                action: CONTENT_ACTIONS.STOP_PROMPT,
                itemIds: ids
            }, { frameId: 0 }, () => resolve());
        })));
    } catch {
...
async function ensureAutomationInjected(tabId, options = {}) {
    ...
    if (!force) {
        const alreadyReady = await new Promise((resolve) => {
            ...
            chrome.tabs.sendMessage(tabId, { action: CONTENT_ACTIONS.PING }, { frameId: 0 }, (response) => {
                if (settled) return;
                settled = true;
                clearTimeout(timer);
...
function sanitizeDownloadBaseName(value = '') {
    const withoutExtension = sanitizePathSegment(value, '')
...
function getDownloadUrlMatchKey(value = '') {
    const url = String(value || '');
    if (!url) return '';
    // Data URLs can be several megabytes. A stable prefix/suffix fingerprint is
    // sufficient for the very short-lived, same-worker reservation window.
    if (url.startsWith('data:')) {
        return `data:${url.length}:${url.slice(0, 96)}:${url.slice(-96)}`;
    }
    return url.split('#')[0];
}

function pruneExtensionDownloadNames(now = Date.now()) {
    for (let index = pendingExtensionDownloadNames.length - 1; index >= 0; index -= 1) {
        if (Number(pendingExtensionDownloadNames[index]?.expiresAt || 0) <= now) {
            pendingExtensionDownloadNames.splice(index, 1);
        }
    }
}

function reserveExtensionDownloadName(url = '', filename = '') {
    pruneExtensionDownloadNames();
    const safeFilename = sanitizeRelativeDownloadPath(filename);
    const urlKey = getDownloadUrlMatchKey(url);
    if (!safeFilename || !urlKey) return null;
    const token = `direct_name_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const entry = {
        token,
        urlKey,
        filename: safeFilename,
        createdAt: Date.now(),
        expiresAt: Date.now() + EXTENSION_DOWNLOAD_NAME_TTL_MS
    };
    pendingExtensionDownloadNames.push(entry);
    return entry;
}

function cancelExtensionDownloadName(token = '') {
    const index = pendingExtensionDownloadNames.findIndex((entry) => entry.token === token);
    if (index !== -1) pendingExtensionDownloadNames.splice(index, 1);
}

function consumeExtensionDownloadName(item = {}) {
    // Redirect-backed Flow media can omit byExtensionId even when this
    // extension initiated the request. Reject an explicit different owner,
    // but allow a missing owner to consume the exact short-lived URL match.
    if (item.byExtensionId && item.byExtensionId !== chrome.runtime.id) return null;
    pruneExtensionDownloadNames();
    const itemUrlKeys = new Set([
        getDownloadUrlMatchKey(item.url),
        getDownloadUrlMatchKey(item.finalUrl)
    ].filter(Boolean));
    const index = pendingExtensionDownloadNames.findIndex((entry) => (
        itemUrlKeys.has(entry.urlKey)
    ));
    if (index === -1) return null;
    const [entry] = pendingExtensionDownloadNames.splice(index, 1);
    if (!entry?.filename) return null;

    // The redirect URL normally has no suffix. By filename-determination time
    // Chrome knows the response MIME and/or Flow's tentative UUID.jpeg name.
    // Keep the requested base/path but use that real media extension so JPEG or
    // WebP bytes are never mislabeled as .png.
    const mimeExtension = inferExtensionFromContentType(item.mime || '');
    const tentativeBase = String(item.filename || '').split(/[\\/]/).pop() || '';
    const tentativeMatch = tentativeBase.match(/\.([a-z0-9]{1,8})$/i);
    const tentativeExtension = tentativeMatch?.[1]?.toLowerCase() || '';
    const supportedExtension = /^(?:png|jpe?g|webp|gif|avif|mp4|webm|mov|m4v)$/i;
    const actualExtension = supportedExtension.test(mimeExtension || '')
        ? mimeExtension
        : (supportedExtension.test(tentativeExtension) ? tentativeExtension : '');
    if (actualExtension) {
        entry.filename = entry.filename.replace(/\.[a-z0-9]{1,8}$/i, '') + `.${actualExtension}`;
    }
    entry.resolvedFilename = entry.filename;
    return entry;
}

function buildBulkDownloadBaseName(baseName = '', index = 1) {
    const base = sanitizeDownloadBaseName(baseName) || 'GoogleFlowAutomator';
    const safeIndex = Math.max(1, Number.parseInt(index, 10) || 1);
    return `${base}${String(safeIndex - 1).padStart(3, '0')}`;
}

async function persistPendingNativeDownloadNames() {
    const now = Date.now();
    const active = pendingNativeDownloadNames.filter((entry) => (
        entry?.relativeBase && Number(entry.expiresAt) > now
    ));
    pendingNativeDownloadNames.splice(0, pendingNativeDownloadNames.length, ...active);
    await chrome.storage.session.set({
        [PENDING_NATIVE_DOWNLOAD_NAMES_STORAGE_KEY]: active
    });
}

async function registerBulkNativeDownloadName(message = {}, sender = {}) {
    await pendingNativeDownloadNamesReady;
    const source = message.source === 'pageDownloader' || message.source === 'historyDownloader'
        ? message.source
        : 'generated';
    // Generated 2K download + custom queue name is Premium. The standalone
    // bulk Image Downloader remains Professional/Supporter-only.
    const namingUnlocked = source === 'generated'
        ? await isUpscaledGeneratedDownloadUnlocked()
        : await isPremiumImageDownloaderUnlocked();
    if (!namingUnlocked) {
        return { ok: false, error: PREMIUM_FEATURE_LOCKED_MESSAGE };
    }

    const [queue, storedSettings] = await Promise.all([
        storage.getQueue(),
        storage.getSettings()
    ]);
    const trustedItemId = String(message.queueItemId || '');
    const targetPrompt = normalizePromptForMatch(message.prompt || '');
    const matchedItem = queue.find((item) => trustedItemId && String(item?.id || '') === trustedItemId)
        || (targetPrompt
            ? queue.find((item) => {
                const itemPrompt = normalizePromptForMatch(item?.prompt || '');
                return itemPrompt && (
                    itemPrompt.includes(targetPrompt.slice(0, 30))
                    || targetPrompt.includes(itemPrompt.slice(0, 30))
                );
            })
            : null);
    const queueName = sanitizeDownloadBaseName(matchedItem?.outputName || '');
    // Generated requests must use the stored queue name. Page/history requests
    // are Professional-only and may carry their exact scanned-card mapping.
    const requestedName = source === 'generated'
        ? ''
        : sanitizeDownloadBaseName(message.fileBaseName || '');
    const storedBulkBaseName = sanitizeDownloadBaseName(storedSettings?.downloadBulkBaseName || '');
    const fallbackName = buildBulkDownloadBaseName(storedBulkBaseName, message.bulkIndex);
    // The popup already matched each selected card to its exact requested
    // name. Never overwrite that explicit value with a fuzzy prompt match.
    const fileBaseName = source === 'generated'
        ? (queueName || fallbackName)
        : (requestedName || queueName || fallbackName);
    if (!fileBaseName) return { ok: false, error: 'No custom download name is set.' };
    const projectFolder = sanitizePathSegment(
        normalizeFlowProjectTitle(message.projectName || ''),
        'Flow_Exports'
    );

    const token = `bulk_name_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const pendingEntry = {
        token,
        relativeBase: `${projectFolder}/${fileBaseName}`,
        createdAt: Date.now(),
        mediaType: message.mediaType === 'video' ? 'video' : 'image',
        source,
        expiresAt: Date.now() + NATIVE_DOWNLOAD_NAME_TTL_MS
    };
    pendingNativeDownloadNames.push(pendingEntry);
    try {
        await persistPendingNativeDownloadNames();
    } catch (error) {
        const rollbackIndex = pendingNativeDownloadNames.findIndex((entry) => entry.token === token);
        if (rollbackIndex !== -1) pendingNativeDownloadNames.splice(rollbackIndex, 1);
        throw error;
    }
    return { ok: true, token, fileBaseName };
}

async function cancelBulkNativeDownloadName(token = '') {
    await pendingNativeDownloadNamesReady;
    const index = pendingNativeDownloadNames.findIndex((entry) => entry.token === token);
    if (index !== -1) pendingNativeDownloadNames.splice(index, 1);
    await persistPendingNativeDownloadNames();
}
...
function normalizeFlowProjectTitle(value = '') {
    return String(value || '')
...
function getProjectFolderName(message, sender) {
    const explicitName = sanitizePathSegment(
        normalizeFlowProjectTitle(message?.projectName || ''),
        ''
    );
    ...
    const normalizedTitle = normalizeFlowProjectTitle(tabTitle);
...
function inferExtensionFromContentType(contentType = '') {
    ...
    if (ct.includes('video/x-m4v') || ct.includes('video/m4v')) return 'm4v';
...
async function saveDownloadAndTrack(message, sender, defaultBaseName = 'flow_result', options = {}) {
    ...
    const requestedFilename = message.filename
        ? sanitizeRelativeDownloadPath(message.filename)
        : '';

    // Build filename:
    // - Prefer scene number from prompt prefix ("12: ...") or queueIndex
    // - Include a project sequence only when the caller did not supply a name
    // A caller-supplied filename must not depend on counter bookkeeping; the
    // download should still start even when chrome.storage is unavailable.
    let sequence = null;
    let filename = requestedFilename;
    if (!filename) {
        try {
            sequence = await storage.getNextDownloadNumber(projectFolder);
        } catch (counterError) {
            sequence = Number.isInteger(message.queueIndex)
                ? message.queueIndex + 1
                : (Date.now() % 1000000);
            console.warn('[FlowAutomator] Download counter unavailable; using a fallback sequence.', counterError);
        }
    ...
    if (!filename) {
        filename = `${projectFolder}/${defaultBaseName}.${ext}`;
    }
    const downloadSource = message.dataUrl || message.url;

    // Reserve before starting the request. Flow media endpoints may supply a
    // UUID Content-Disposition name; the downloads event below reasserts our
    // exact queue/bulk name even when Chrome initially exposes that UUID.
    const directNameReservation = reserveExtensionDownloadName(downloadSource, filename);
    let downloadId;
    try {
        ...
        cancelExtensionDownloadName(directNameReservation?.token || '');
        throw error;
    }
    if (directNameReservation?.resolvedFilename) {
        filename = directNameReservation.resolvedFilename;
    }
    ...
    const record = {
        ...
        mediaType: String(message.mediaType || '').toLowerCase() || 'image',
    ...
    if (message.dataUrl) {
        ...
        // Payload embedding is optional post-download metadata. It must never
        // turn an already-started Chrome download into a reported failure.
        try {
            const payload = await fetchMediaAsDataUrl(message.url);
            record.contentType = payload.contentType;
            record.dataUrl = payload.dataUrl;
        } catch (payloadError) {
            console.warn('[FlowAutomator] Could not embed the downloaded payload.', payloadError);
        }
    }

    // Temp-download persistence was removed from shared/storage.js. Keeping the
    // obsolete call here made successful downloads report as failures and caused
    // the page collector to retry them. The returned record is the only consumer.
    return record;
}
...
function hasFlowUuidDownloadName(filename = '') {
    const baseName = String(filename || '').split(/[\\/]/).pop() || '';
    return /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}(?: \(\d+\))?\.[a-z0-9]{1,8}$/i.test(baseName);
}

function hasFlowTimestampDownloadName(filename = '') {
    const baseName = String(filename || '').split(/[\\/]/).pop() || '';
    // Current Flow native downloads use a prompt-derived prefix followed by a
    // compact timestamp, optionally with a 2K/4K marker.
    return /_(?:[24]k_)?20\d{10,12}(?: \(\d+\))?\.(?:png|jpe?g|webp|gif|avif)$/i.test(baseName);
}

// Intercept native Flow downloads. Manual bulk-upscale names take priority;
// generated queue downloads then fall back to the active item's outputName.
async function determineFlowDownloadFilename(item) {
    await pendingNativeDownloadNamesReady;
    // Never compete with a download explicitly initiated by another extension.
    // A missing owner is still eligible because Chrome omits byExtensionId for
    // some redirect-backed Flow downloads.
    if (item.byExtensionId && item.byExtensionId !== chrome.runtime.id) return null;
    const isNative = !item.byExtensionId;
    const referrer = (item.referrer || '').toLowerCase();
    const url = (item.url || '').toLowerCase();
    const finalUrl = (item.finalUrl || '').toLowerCase();
    const isFlowDownload = referrer.includes('flow.google') ||
                           ...
                           finalUrl.includes('google.com/labs/flow') ||
                           finalUrl.includes('labs.google') ||
                           finalUrl.includes('flow.google') ||
                           finalUrl.includes('googlevideo.com');

    const originalFilename = item.filename || '';
    const parts = originalFilename.split('.');
    const rawExt = parts.length > 1 ? parts.pop().toLowerCase() : 'png';
    const ext = /^[a-z0-9]{1,8}$/.test(rawExt) ? rawExt : 'png';
    const now = Date.now();
    while (pendingNativeDownloadNames.length && pendingNativeDownloadNames[0].expiresAt <= now) {
        pendingNativeDownloadNames.shift();
    }

    // DownloadItem has no tabId. Match the short-lived reservation against
    // Flow signals plus its UUID-style native filename/media type instead of
    // allowing an unrelated browser download to consume the first name.
    const hasFlowUuidName = hasFlowUuidDownloadName(originalFilename);
    const hasFlowTimestampName = hasFlowTimestampDownloadName(originalFilename);
    const mime = String(item.mime || '').toLowerCase();
    const looksLikeImage = mime.startsWith('image/') || /^(?:png|jpe?g|webp|gif|avif)$/i.test(ext);
    const looksLikeVideo = mime.startsWith('video/') || /^(?:mp4|webm|mov|m4v)$/i.test(ext);
    const isLikelyReservedFlowDownload = isFlowDownload || hasFlowUuidName || hasFlowTimestampName;
    const canConsumeReservation = !item.byExtensionId || item.byExtensionId === chrome.runtime.id;
    if (canConsumeReservation && isLikelyReservedFlowDownload && pendingNativeDownloadNames.length) {
        const pendingIndex = pendingNativeDownloadNames.findIndex((entry) => {
            const ageMs = now - Number(entry.createdAt || 0);
            if (ageMs < -5000 || ageMs > NATIVE_DOWNLOAD_MATCH_WINDOW_MS) return false;
            return entry.mediaType === 'video' ? looksLikeVideo : looksLikeImage;
        });
        if (pendingIndex !== -1) {
            const [pendingName] = pendingNativeDownloadNames.splice(pendingIndex, 1);
            const relativeBase = sanitizeRelativeDownloadPath(pendingName?.relativeBase || '');
            // Persistence cleanup is best-effort. A storage.session failure must
            // not throw away the filename suggestion for the current download.
            try {
                await persistPendingNativeDownloadNames();
            } catch (error) {
                console.warn('[Downloads] Could not persist consumed filename reservation:', error);
            }
            if (relativeBase) {
                return { filename: `${relativeBase}.${ext}`, conflictAction: 'uniquify' };
            }
        }
    }

    if (isNative && isFlowDownload) {
        if (activeQueueItems.size > 0) {
                ...
                const outputName = matchedEntry?.customFilenameUnlocked
                    ? sanitizeDownloadBaseName(capturedItem?.outputName || '')
                    : '';

                if (outputName) {
                    const explicitFolder = sanitizePathSegment(
                        normalizeFlowProjectTitle(
                            matchedEntry?.projectName || capturedItem.projectName || ''
                        ),
                        ''
                    );
...
chrome.downloads.onDeterminingFilename.addListener((item, suggest) => {
    // Names passed to chrome.downloads.download() are also held in a short-lived
    // reservation so a Flow UUID/Content-Disposition value cannot win. This is
    // synchronous and takes precedence over page-triggered upscale reservations.
    const directName = consumeExtensionDownloadName(item);
    if (directName?.filename) {
        suggest({ filename: directName.filename, conflictAction: 'uniquify' });
        return false;
    }
    // Flow can attribute its page-triggered 2K/4K download to this extension
    // and use a non-UUID timestamp filename. Always let it reach the guarded
    // reservation matcher instead of accepting Flow's random/default name.
    determineFlowDownloadFilename(item)
        .then((suggestion) => suggestion || null)
        .catch((error) => {
            console.warn('[Downloads] Filename resolution failed:', error);
            return null;
        })
        .then((suggestion) => {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    switch (message.action) {
        ...
        case 'DOWNLOAD_HISTORY':
            ...
            return true;
        case 'REGISTER_BULK_DOWNLOAD_NAME':
            registerBulkNativeDownloadName(message, sender)
                .then((result) => sendResponse(result))
                .catch((error) => sendResponse({ ok: false, error: error?.message || String(error) }));
            return true;
        case 'CANCEL_BULK_DOWNLOAD_NAME':
            cancelBulkNativeDownloadName(message.token || '')
                .then(() => sendResponse({ ok: true }))
                .catch((error) => sendResponse({ ok: false, error: error?.message || String(error) }));
            return true;
        case 'DOWNLOAD_PAGE':
            (async () => {
                try {
                    ...
                    startPageDownload(message.tabId, {
                        ...
                        bulkBaseName: message.bulkBaseName,
                        downloadNameEntries: message.downloadNameEntries,
            ...
            (async () => {
                try {
                    ...
                    if (membership.disabled) {
                        ...
                        safeSendMessage({ action: 'UPDATE_PROGRESS', itemId: 'account_disabled_gate', detail: error });
                ...
                (async () => {
                    ...
                    if (freshItem && freshItem.status === QUEUE_STATUS.IN_PROGRESS) {
                        ...
                        if (message.result?.success) {
                            ...
                            const savedName = await isPerPromptAssetsUnlocked()
                                ? sanitizeDownloadBaseName(freshItem.outputName || '')
                                : '';
        ...
        case 'DOWNLOAD_RESULT':
            {
                const dataUrlKey = message.dataUrl
                    ? `data_${String(message.dataUrl).length}_${String(message.dataUrl).slice(0, 48)}_${String(message.dataUrl).slice(-48)}`
                    : '';
                const downloadSourceKey = message.tileId || message.url || dataUrlKey;
                if (!downloadSourceKey || (!message.url && !message.dataUrl)) {
                    sendResponse({ ok: false, error: 'No download source was provided.' });
                    return true;
                }
                const downloadRequestKey = `${message.downloadRunId || 'generated'}:${downloadSourceKey}`;
                runDownloadRequestOnce(downloadRequestKey, async () => {
                try {
                    if (!(await isImageDownloaderUnlocked())) {
                        await blockImageDownloaderAction('Generated auto-download');
                        return { ok: false, error: PREMIUM_FEATURE_LOCKED_MESSAGE };
                    }
                    if (!sessionSeenImages.has(downloadRequestKey)) {
                        const dlMessage = { ...message };
                        // Never trust a caller-provided filename. Queue names and
                        // Premium bulk names are resolved from extension storage
                        // below, so runtime-message injection cannot bypass tiers.
                        dlMessage.filename = '';
                        const [queue, storedSettings] = await Promise.all([
                            storage.getQueue(),
                            storage.getSettings()
                        ]);
                        const trustedQueueItemId = String(message.queueItemId || message.downloadRunId || '');
                        const queueItem = queue.find((item) => (
                            trustedQueueItemId && String(item?.id || '') === trustedQueueItemId
                        )) || (message.queueIndex != null ? queue[message.queueIndex] : null);
                        // Per-queue names are Starter+, but the background reads
                        // the stored queue value instead of trusting a content
                        // message that can be forged from DevTools.
                        const queueOutputName = sanitizeDownloadBaseName(queueItem?.outputName || '');
                        if (queueOutputName) {
                            const dlExt = inferFileExtension(message);
                            const projectFolder = getProjectFolderName(message, sender);
                            dlMessage.filename = `${projectFolder}/${queueOutputName}.${dlExt}`;
                        }
                        if (!dlMessage.filename) {
                            // A shared bulk base is Premium+. Re-check it in the
                            // service worker so a Starter cannot force one via
                            // DevTools/runtime messaging. The per-queue name
                            // above remains available to Starter.
                            const bulkNamingUnlocked = await isBulkDownloadBaseNameUnlocked();
                            const allowedBulkBaseName = bulkNamingUnlocked
                                ? sanitizeDownloadBaseName(storedSettings?.downloadBulkBaseName || '')
                                : '';
                            const fallbackName = buildBulkDownloadBaseName(
                                allowedBulkBaseName,
                                message.bulkIndex ?? ((message.queueIndex ?? 0) + 1)
                            );
                            const fallbackExt = inferFileExtension(dlMessage);
                            const fallbackProjectFolder = getProjectFolderName(dlMessage, sender);
                            dlMessage.filename = `${fallbackProjectFolder}/${fallbackName}.${fallbackExt}`;
                        }
                        const record = await saveDownloadAndTrack(dlMessage, sender, 'flow_result');
                        rememberCompletedDownloadRequest(downloadRequestKey);
                        return { ok: true, filename: record?.filename || dlMessage.filename || '' };
                    }
                    ...
                    (result) => {
                        try { sendResponse(result); } catch (responseError) {
                            console.warn('DOWNLOAD_RESULT response failed:', responseError);
                        }
                    },
                    (error) => {
                        console.error('DOWNLOAD_RESULT request failed unexpectedly:', error);
                        try { sendResponse({ ok: false, error: error?.message || String(error) }); } catch (responseError) {
                            console.warn('DOWNLOAD_RESULT error response failed:', responseError);
                        }
                    }
                );
                return true;
        ...
        case 'AUTO_COLLECT_DOWNLOAD':
            {
                const mediaKey = message.tileId || message.url || '';
                if (!message.url || !mediaKey) {
                    sendResponse({ ok: false, error: 'No download source was provided.' });
                    return true;
                }
                const downloadRequestKey = `${message.downloadRunId || 'page'}:${mediaKey}`;
                runDownloadRequestOnce(downloadRequestKey, async () => {
                try {
                    if (!(await isPremiumImageDownloaderUnlocked())) {
                    ...
                    if (!sessionSeenImages.has(downloadRequestKey)) {
                        // Exact card names do not depend on queue storage. Only
                        // consult fuzzy prompt matching when no explicit name was
                        // supplied by the downloader.
                        const explicitBaseName = sanitizeDownloadBaseName(message.fileBaseName || '');
                        let queue = [];
                        let matchIdx = -1;
                        if (!explicitBaseName) {
                            queue = await storage.getQueue();
                            const targetMsgPrompt = normalizePromptForMatch(message.prompt);
                            matchIdx = targetMsgPrompt
                        ...
                        const enriched = matchIdx >= 0
                            ...
                            : { ...message };
                        // The downloader already matched the selected card to its
                        // exact requested name. Never let a fuzzy prompt match
                        // overwrite that explicit name (similar prompts can map to
                        // the wrong queue row).
                        if (explicitBaseName) {
                            const explicitExt = inferFileExtension(enriched);
                            const explicitProjectFolder = getProjectFolderName(enriched, sender);
                            enriched.filename = `${explicitProjectFolder}/${explicitBaseName}.${explicitExt}`;
                        } else if (matchIdx >= 0) {
                            const acOutputName = sanitizeDownloadBaseName(queue[matchIdx]?.outputName || '');
                            if (acOutputName) {
                        ...
                        if (!enriched.filename) {
                            const storedSettings = await storage.getSettings();
                            const storedBulkBaseName = sanitizeDownloadBaseName(storedSettings?.downloadBulkBaseName || '');
                            const bulkName = buildBulkDownloadBaseName(storedBulkBaseName, enriched.bulkIndex);
                            if (bulkName) {
                                const bulkExt = inferFileExtension(enriched);
                                const bulkProjectFolder = getProjectFolderName(enriched, sender);
                                enriched.filename = `${bulkProjectFolder}/${bulkName}.${bulkExt}`;
                            }
                        }
                        const record = await saveDownloadAndTrack(enriched, sender, 'flow_gen');
                        rememberCompletedDownloadRequest(downloadRequestKey);
                        return { ok: true, filename: record?.filename || enriched.filename || '' };
                    }
                    return { ok: true, skipped: true };
                } catch (e) {
                    ...
                    (result) => {
                        try { sendResponse(result); } catch (responseError) {
                            console.warn('AUTO_COLLECT_DOWNLOAD response failed:', responseError);
                        }
                    },
                    (error) => {
                        console.error('AUTO_COLLECT_DOWNLOAD request failed unexpectedly:', error);
                        try { sendResponse({ ok: false, error: error?.message || String(error) }); } catch (responseError) {
                            console.warn('AUTO_COLLECT_DOWNLOAD error response failed:', responseError);
                        }
                    }
                );
                return true;
...
async function startQueue() {
    ...
    const membership = await getMembershipState();
    if (membership.disabled) {
        const error = getLocalizedAccountDisabledMessage(settings);
        await storage.addLog(error, 'error');
    ...
    if (membership.tier === 'starter' && membership.uid) {
        ...
        if (usage && usage.count >= STARTER_ACCOUNT_PROMPT_LIMIT) {
            await storage.addLog(getStarterQuotaReachedMessage(), 'error');
            await storage.setState(AUTOMATOR_STATE.STOPPED);
            safeSendMessage({ action: 'UPDATE_PROGRESS', itemId: 'starter_quota_gate', detail: getStarterQuotaReachedMessage() });
            return;
        }
    }
...
async function startHistoryDownload(preferredTabId = null) {
    ...
    chrome.tabs.sendMessage(targetTabId, {
        action: CONTENT_ACTIONS.DOWNLOAD_HISTORY,
...
async function collectPageDownloadImages(preferredTabId = null) {
    ...
    return new Promise((resolve) => {
        chrome.tabs.sendMessage(targetTab.id, {
            action: CONTENT_ACTIONS.COLLECT_PAGE_IMAGES,
            payload: { selectors }
        }, (response) => {
            if (chrome.runtime.lastError) {
...
async function startPageDownload(preferredTabId = null, options = {}) {
    ...
    chrome.tabs.sendMessage(targetTabId, {
        action: CONTENT_ACTIONS.DOWNLOAD_PAGE,
        payload: {
            settings,
            selectors,
            selectedIds: Array.isArray(options.selectedIds) ? options.selectedIds : [],
            bulkBaseName: sanitizePathSegment(options.bulkBaseName || '', ''),
            downloadNameEntries: (Array.isArray(options.downloadNameEntries) ? options.downloadNameEntries : [])
                .map((entry) => ({
                    id: String(entry?.id || ''),
                    name: sanitizeDownloadBaseName(entry?.name || '')
                }))
    ...
    while (true) {
        ...
        if (currentIdx === -1) {
            if (inProgressCount > 0) {
                ...
                await chrome.alarms.clear('queue-keepalive');
                isLooping = false;
                break;
            }
        }
        ...
        if (membershipForDelay.tier === 'starter' && membershipForDelay.uid) {
            ...
            if (usage && usage.count >= STARTER_ACCOUNT_PROMPT_LIMIT) {
                const quotaReachedMessage = getStarterQuotaReachedMessage();
                await storage.addLog(quotaReachedMessage, 'error');
                safeSendMessage({ action: 'UPDATE_PROGRESS', itemId: 'starter_quota_gate', detail: quotaReachedMessage });
                await storage.setState(AUTOMATOR_STATE.STOPPED);
                isLooping = false;
                break;
            }
...
async function executePrompt(item, settings) {
    ...
    let effectiveSettings = {
        ...
        // Flow's composer model/ratio/quantity controls are Professional-only.
        // Premium and Starter runs must preserve whatever is already selected
        // on the Flow page and submit the prompt without opening that panel.
        syncFlowSettings: membershipState.tier === 'professional',
    ...
    // Queue-by-queue file names are available from Starter. Premium gating is
    // only for the shared bulk base name and unrelated per-prompt assets.
    const queueOutputName = sanitizeDownloadBaseName(item.outputName || '');
    effectiveSettings.downloadBulkBaseName = perPromptAssetsUnlocked
        ? sanitizeDownloadBaseName(settings?.downloadBulkBaseName || '')
        : '';
    const activeDownloadEntry = activeQueueItems.get(item.id);
    if (activeDownloadEntry) {
        activeDownloadEntry.customFilenameUnlocked = downloaderUnlocked;
        activeDownloadEntry.projectName = normalizeFlowProjectTitle(targetTab?.title || '');
    }
    ...
    if (perPromptAssetsUnlocked && autoMentionUnlocked && effectiveSettings.autoMentionEnabled) {
        ...
        if (promptHasMention && matchedRefs === 0) {
            ...
            // source of truth even when Flow's on-page rename or referenceAssets
            // registration failed. Reuse the completed item's resultUrl so @mentions
            // still work when Flow's media-name search is unavailable or stale.
    ...
    const pingAction = isVideoRun ? CONTENT_ACTIONS.PING_VIDEO : CONTENT_ACTIONS.PING;
    const submitAction = isVideoRun ? CONTENT_ACTIONS.SUBMIT_VIDEO_PROMPT : CONTENT_ACTIONS.SUBMIT_PROMPT;
    ...
    return new Promise((resolve) => {
        ...
        (async () => {
            ...
            const sanitizedVideoSettings = isVideoRun ? sanitizeVideoSettings({
                ...
                videoEndFrameSelection: usePerPromptAssets
                    ? (item.videoEndFrameSelection || null)
                    : (effectiveSettings.videoEndFrameSelection || null),
            ...
            const payload = {
                ...
                payload: {
                    ...
                    settings: {
                        ...
                        // Queue-level video assets only override the global
                        // source when per-prompt asset mode is enabled.
                        videoStartImage: usePerPromptAssets
                            ? (item.videoStartImage || null)
                            : (effectiveSettings.videoStartImage || null),
                        ...
                        ...(isVideoRun && sanitizedVideoSettings ? {
                            ...
                            videoIngredientSelections: usePerPromptAssets
                                ? (item.videoIngredientSelections || [])
                                : (effectiveSettings.videoIngredientSelections || []),
                            videoStartImage: usePerPromptAssets
                                ? (item.videoStartImage || item.videoStartFrameSelection || null)
                                : (effectiveSettings.videoStartImage || null),
                            videoStartFrameSelection: usePerPromptAssets
                                ? (item.videoStartFrameSelection || null)
                                : (effectiveSettings.videoStartFrameSelection || null),
            ...
            if (isVideoRun && payload.payload.settings?.dryRunVideoSetup === true) {
                ...
                const dryRunResult = await sendTabMessageWithTimeout(targetTabId, {
                    ...payload,
                    action: CONTENT_ACTIONS.VIDEO_DRY_RUN
                }, 0, 120000);
                const fn = pendingResolves.get(item.id);
            ...
            await sendTabMessage(targetTabId, { action: CONTENT_ACTIONS.CLEAR_PROMPT_STOP, itemId: item.id }, 0);
            if (!(await isQueueStillRunning()) || !pendingResolves.has(item.id)) {
                return;
            }
