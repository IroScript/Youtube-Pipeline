...
if (window.__FLOW_AUTOMATOR_ACTIVE_SUBMISSION__ === true) {
    // Keep the active submission and ignore duplicate injection silently.
} else {
    ...
    (async () => {
        const CONTENT_ACTIONS = Object.freeze({
            PING: 'GFA_V222_14_PING',
            PING_VIDEO: 'GFA_V222_14_PING_VIDEO',
            SUBMIT_PROMPT: 'GFA_V222_14_SUBMIT_PROMPT',
            SUBMIT_VIDEO_PROMPT: 'GFA_V222_14_SUBMIT_VIDEO_PROMPT',
            VIDEO_DRY_RUN: 'GFA_V222_14_VIDEO_DRY_RUN',
            CLEAR_PROMPT_STOP: 'GFA_V222_14_CLEAR_PROMPT_STOP',
            STOP_PROMPT: 'GFA_V222_14_STOP_PROMPT',
            DOWNLOAD_HISTORY: 'GFA_V222_14_DOWNLOAD_HISTORY',
            DOWNLOAD_PAGE: 'GFA_V222_14_DOWNLOAD_PAGE',
            COLLECT_PAGE_IMAGES: 'GFA_V222_14_COLLECT_PAGE_IMAGES'
        });
        ...
        async function tryClickElementViaBridge(el, { preferClick = false } = {}) {
            if (!el) return false;
            ...
            return new Promise(function (resolve) {
                ...
                try {
                    document.dispatchEvent(new CustomEvent('FLOW_AUTOMATOR_CLICK_ELEMENT', {
                        detail: { requestId: reqId, targetId: elId, preferClick }
                    }));
                } catch (e) {
                    settled = true;
                    resolve(false);
        ...
        function releaseFocusFromAriaHiddenPopup(popupRoot = null) {
            try {
                const active = document.activeElement;
                if (!active || active === document.body) return false;
                const hiddenAncestor = active.closest?.(
                    '[aria-hidden="true"], [data-aria-hidden="true"], [inert]'
                );
                const retainedInsideClosingPopup = !!popupRoot && popupRoot.contains?.(active);
                if (!hiddenAncestor && !retainedInsideClosingPopup) return false;
                active.blur?.();
                console.info('[FlowAutomator][Rename] released-focus-from-hidden-popup');
                return true;
            } catch (e) {
                return false;
            }
        ...
        function findUpscaleButton(scope, targetQuality = '2k') {
            ...
            // Flow may render a quality submenu entry as only "2K" or "4K".
            // Accept a bare quality label only when it is an actual visible menu
            // item, never a badge elsewhere in the media tile.
            const bareQualityMenuItem = candidates.find((el) => {
                const txt = getNodeText(el);
                const otherQuality = quality === '2k' ? '4k' : '2k';
                const isMenuEntry = el.matches?.('[role="menuitem"], [data-radix-collection-item]')
                    || !!el.closest?.('[role="menu"]');
                return isMenuEntry && txt.includes(quality) && !txt.includes(otherQuality);
            });
            if (bareQualityMenuItem) return bareQualityMenuItem;
        ...
        function findDownloadMenuItem(scope = document) {
            ...
            const visibleMenuItem = candidates.find(({ el }) => (
                el.matches?.('[role="menuitem"]') || !!el.closest?.('[role="menu"]')
            ));
            if (visibleMenuItem) return visibleMenuItem.el;
        ...
        function hasOpenDownloadMenu() {
            return getVisibleMenuEntries().some((el) => {
                const txt = getNodeText(el);
        ...
        async function openOverflowMenuForImage(img, tile, updateStatus, currentIndex) {
            const isRenameOperation = String(currentIndex || '').toLowerCase() === 'rename';
            const isMenuOpen = () => isRenameOperation
                ? !!findRenameMenuItem(document)
                : hasOpenDownloadMenu();
            for (let attempt = 0; attempt < 3; attempt += 1) {
                ...
                for (const btn of candidates) {
                    ...
                    // The Flow media overflow trigger is React-controlled. For
                    // rename operations, a synthetic content-script pointer
                    // sequence can locate the correct button but fail to open
                    // the Radix menu. Repeat the activation in the page world,
                    // matching the console-proven path.
                    if (isRenameOperation && document.contains(btn)) {
                        const openedViaBridge = await tryClickElementViaBridge(btn);
                        if (openedViaBridge
                            && await waitForCondition(isMenuOpen, 2500, 150)) {
                            return true;
                        }
                    }
                }
        ...
        async function tryUpscaleThenAutoDownload(img, { updateStatus = null, currentIndex = 0, upscaleQuality = '2k' } = {}) {
            ...
            if (!upscaleBtn) {
                // A Professional extension account can request 4K while the
                // connected Flow account exposes 4K as a disabled Upgrade item.
                // In that case use the available 2K *upscaled* option. Never
                // fall back to 1K/original while Upscaled Download is enabled.
                const twoKFallback = quality === '4k'
                    ? findUpscaleButton(submenuEntries, '2k')
                    : null;
                if (twoKFallback) {
                    upscaleBtn = twoKFallback;
                    selectedQuality = '2k';
                    if (typeof updateStatus === 'function') {
                        updateStatus(`4K is not available in this Flow account; using 2K Upscaled for ${currentIndex}.`);
                    }
                }
            }
            ...
            if (!upscaleBtn) {
                if (typeof updateStatus === 'function') {
                    updateStatus(`${qualityLabel} Upscaled is unavailable for ${currentIndex}; original quality was not downloaded.`);
                }
                document.body?.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', code: 'Escape', bubbles: true }));
                return false;
            }
            ...
            if (typeof updateStatus === 'function') {
                updateStatus(`Upscaling ${currentIndex} to ${selectedQuality.toUpperCase()}... waiting for auto-download`);
            }
            const completed = await waitForUpscaleCompleteAndDismiss({ timeoutMs: 180000, baselineNotices });
            return completed ? selectedQuality : false;
        }

        async function directDownloadItem(img, itemId, {
            ...
            queueItemId = '',
            downloadRunId = '',
            nativeDownloadSource = 'generated'
        } = {}) {
            const mediaUrl = getMediaSourceUrl(img);
            ...
            if (preferUpscaledDownload && !isVideo) {
                const namingResult = await requestRuntimeMessage({
                    action: 'REGISTER_BULK_DOWNLOAD_NAME',
                    prompt: promptText,
                    projectName,
                    bulkBaseName,
                ...
                const nativeNameToken = namingResult?.ok ? namingResult.token : '';
                // Never start a Flow-native download without an acknowledged
                // filename reservation. Otherwise Chrome keeps Flow's UUID and
                // the UI incorrectly reports success.
                if (!nativeNameToken) {
                    const namingError = namingResult?.error || 'The filename reservation did not reach the extension.';
                    console.warn('[FlowAutomator] Native download blocked because filename reservation failed:', namingError);
                    if (typeof updateStatus === 'function') {
                        updateStatus(`Download stopped before saving: ${namingError}`);
                    }
                    return false;
                }
                const downloadedUpscaleQuality = await tryUpscaleThenAutoDownload(img, { updateStatus, currentIndex, upscaleQuality });
                if (downloadedUpscaleQuality) {
                    // Keep the reserved name until onDeterminingFilename
                    // consumes it. Flow can emit the actual native download
                    // well after the upscale-complete notice appears.
                    seenImages.add(itemId);
                    return downloadedUpscaleQuality;
                }
                if (nativeNameToken) {
                    safeSendMessage({ action: 'CANCEL_BULK_DOWNLOAD_NAME', token: nativeNameToken });
                }
                if (typeof updateStatus === 'function') {
            ...
            const downloadResult = await requestRuntimeMessage({
            ...
            if (!downloadResult?.ok) {
                sentDownloadKeys.delete(downloadKey);
                if (typeof updateStatus === 'function') {
                    updateStatus(`Download failed for ${currentIndex}: ${downloadResult?.error || 'No response'}`);
                }
                return false;
            }
            if (typeof updateStatus === 'function' && downloadResult.filename) {
                updateStatus(`Downloading as ${downloadResult.filename}`);
            }
        ...
        function findRenameMenuItem(scope = document) {
            ...
            const allElements = Array.from(scope.querySelectorAll('*')).filter(isVisibleRenameElement);
        ...
        function getVisibleRenameInputCandidates() {
            return Array.from(document.querySelectorAll(
                'input, textarea, [contenteditable="true"], [contenteditable="plaintext-only"]'
            )).filter((el) => {
                if (!isVisibleRenameElement(el)) return false;
                const placeholder = String(el.getAttribute?.('placeholder') || '').toLowerCase();
                const ariaLabel = String(el.getAttribute?.('aria-label') || '').toLowerCase();
                const id = String(el.id || '').toLowerCase();
                const isPromptInput = el.matches(
                    '[aria-multiline="true"], [data-lexical-editor="true"], .prompt-input *, .editor-container *'
                ) || placeholder.includes('prompt') || placeholder.includes('describe')
                    || placeholder.includes('what do you want to create');
                const isSearchInput = el.type === 'search' || placeholder.includes('search')
                    || placeholder.includes('검색') || id.includes('search') || ariaLabel === 'search';
                return !isPromptInput && !isSearchInput;
            });
        }
        ...
        // Flow renders the media rename popover in a Radix portal whose visible
        // dialog, textbox, and buttons all have offsetParent === null. The general
        // asset visibility helper intentionally rejects that layout, so rename
        // controls need a geometry/style based visibility contract of their own.
        function isVisibleRenameElement(el) {
        ...
        function findRenameInputField(img, baselineInputs = null) {
            const baseline = baselineInputs instanceof Set ? baselineInputs : new Set();
            const tile = img && document.contains(img) ? getTileContainerForMedia(img) : null;
            const tileRect = tile?.getBoundingClientRect?.() || null;
            const candidates = getVisibleRenameInputCandidates()
                .map((el) => {
                    const rect = el.getBoundingClientRect?.() || null;
                    const nearTargetTile = !tileRect || !rect || (
                        rect.right >= tileRect.left - 24
                        && rect.left <= tileRect.right + 24
                        && rect.bottom >= tileRect.top - 24
                        && rect.top <= tileRect.bottom + 80
                    );
                    let score = baseline.has(el) ? 0 : 120;
                    if (el === document.activeElement) score += 100;
                    if (el.closest('[role="dialog"], dialog, [role="alertdialog"]')) score += 80;
                    if (tile?.contains(el)) score += 50;
                    if (nearTargetTile) score += 160;
                    const contract = `${el.getAttribute?.('aria-label') || ''} ${el.getAttribute?.('placeholder') || ''}`.toLowerCase();
                    if (/rename|name|editable text|이름|名前|名称/.test(contract)) score += 40;
                    if (getEditableValue(el).trim()) score += 10;
                    return { el, score, nearTargetTile };
                })
                .sort((a, b) => b.score - a.score);

            // With a pre-click baseline, require a newly shown/focused/dialog
            // textbox so the project-title textbox can never be renamed.
            const best = candidates.find(({ el, nearTargetTile }) => (
                nearTargetTile
                && (
                    !baselineInputs
                    || !baseline.has(el)
                    || (el === document.activeElement
                        && !!el.closest('[role="dialog"], dialog, [role="alertdialog"]'))
                )
            ));
        ...
        function getRenameCommitScore(btn) {
            if (!btn || !isVisibleRenameElement(btn)) return -1;
            const text = getNodeText(btn).trim();
            const aria = String(btn.getAttribute?.('aria-label') || '').toLowerCase().trim();
            const title = String(btn.getAttribute?.('title') || '').toLowerCase().trim();
            const contract = `${text} ${aria} ${title}`.replace(/\s+/g, ' ').trim();
            // Flow's Material icon text can be concatenated with the visible
            // label when the portal has offsetParent === null. Examples:
            // "doneDone", "checkDone", "saveSave". Match the compact suffix
            // as well as the normal word-separated accessible label.
            const compactContract = contract.replace(/[\s_\-]+/g, '');
            if (/cancel|close|dismiss|clear|취소|닫기|取消|キャンセル/.test(contract)) return -1;
            if (/(?:done|save|confirm|rename|완료|저장|확인|변경|決定|保存|完了)$/.test(compactContract)) return 220;
            if (/^(?:done|done done|save|save save|confirm|ok|rename|완료|저장|확인|변경|決定|保存|完了)(?:\s|$)/.test(contract)) return 200;
            if (/\b(?:done|save|confirm|rename)\b|완료|저장|확인|변경/.test(contract)) return 160;
            if (String(btn.getAttribute?.('type') || '').toLowerCase() === 'submit') return 100;
            return -1;
        }

        function findRenameSurfaceForInput(input) {
            if (!input || !document.contains(input)) return null;
            const explicit = input.closest(
                '[role="dialog"], dialog, [role="alertdialog"], form, .rt-DialogContent, .rt-PopoverContent, .modal-content, .modal, .dialog'
            );
            if (explicit) return explicit;

            // Flow may render an unlabelled styled portal. Walk upward until
            // the smallest common ancestor contains both the textbox and a
            // commit action in a sibling footer.
            let parent = input.parentElement;
            for (let depth = 0; parent && parent !== document.body && depth < 12; depth += 1, parent = parent.parentElement) {
                const hasCommit = Array.from(parent.querySelectorAll('button, [role="button"], [type="submit"]'))
                    .some((btn) => getRenameCommitScore(btn) >= 100);
                if (hasCommit) return parent;
            }
            return null;
        }

        function findRenameSubmitButton(img, preferredInput = null, baselineInputs = null, baselineButtons = null) {
            const input = preferredInput && document.contains(preferredInput) && isVisibleRenameElement(preferredInput)
                ? preferredInput
                : findRenameInputField(img, baselineInputs);
            const surface = findRenameSurfaceForInput(input);
            const baseline = baselineButtons instanceof Set ? baselineButtons : new Set();
            const roots = surface ? [surface] : Array.from(document.querySelectorAll('[role="dialog"], dialog, [role="alertdialog"]'))
                .filter(isVisibleRenameElement);
            const candidates = Array.from(new Set(roots.flatMap((root) => (
                Array.from(root.querySelectorAll('button, [role="button"], [type="submit"]'))
            ))))
                .filter(isVisibleRenameElement)
                .map((btn) => ({
                    btn,
                    score: getRenameCommitScore(btn) + (baseline.has(btn) ? 0 : 20)
                }))
                .filter(({ score }) => score >= 100)
                .sort((a, b) => b.score - a.score);

            if (candidates[0]) return candidates[0].btn;

            // Last resort for a portal whose wrapper has no dialog role: only
            // consider newly appeared document-wide buttons with an explicit
            // Done/Save/Rename contract. Never choose an arbitrary page button.
            const newGlobalAction = Array.from(document.querySelectorAll('button, [role="button"], [type="submit"]'))
                .filter((btn) => !btn.closest('[role="menu"]')
                    && !baseline.has(btn)
                    && getRenameCommitScore(btn) >= 160)
                .sort((a, b) => getRenameCommitScore(b) - getRenameCommitScore(a))[0] || null;
            if (newGlobalAction) return newGlobalAction;

            console.debug('[FlowAutomator][Rename] No commit button matched the active rename textbox yet.');
            return null;
        }
        ...
        async function writeTextToElement(element, text) {
            ...
            try {
                ...
                if (isInput) {
                    const hasVisibleRenameCommitAction = () => (
                        Array.from(document.querySelectorAll('button, [role="button"], [type="submit"]'))
                            .some((button) => (
                                isVisibleRenameElement(button)
                                && !isDisabledButton(button)
                                && getRenameCommitScore(button) >= 100
                            ))
                    );
                    const waitForRenameCommitAction = async (timeoutMs = 700) => {
                        const startedAt = Date.now();
                        while (Date.now() - startedAt < timeoutMs) {
                            if (hasVisibleRenameCommitAction()) return true;
                            await new Promise(r => setTimeout(r, 60));
                        }
                        return hasVisibleRenameCommitAction();
                    };

                    // The rename textbox is React-controlled. Updating it only
                    // through execCommand changes the visible DOM value but can
                    // leave Flow's saved state at the old name. The main-world
                    // bridge updates the native value tracker and calls React's
                    // onChange/onInput handler, matching the verified console flow.
                    element.focus();
                    element.select();
                    try {
                    ...
                    const mainWorldResult = await tryMainWorldSetText(element, text);
                    await new Promise(r => setTimeout(r, 180));
                    const reactValueWritten = mainWorldResult.ok
                        && element.value === text;
                    if (reactValueWritten) {
                        const commitActionReady = await waitForRenameCommitAction(900);
                        console.log(
                            '[FlowAutomator] Rename input updated through React state.',
                            mainWorldResult.info,
                            'Done ready:', commitActionReady
                        );
                        // Flow can temporarily unmount Done when the controlled
                        // textbox re-renders. A correct input value is still a
                        // successful write; the caller reactivates the live input
                        // and resolves the new Done button before submitting.
                        return true;
                    }
                    console.warn(
                        '[FlowAutomator] React bridge did not produce a stable rename value. '
                        + 'Retrying through native browser input events.'
                    );

                    let browserEditSucceeded = false;
                    try {
                        setNativeObjectValue(element, originalValue);
                        element.focus();
                        element.select();
                        browserEditSucceeded = document.execCommand('insertText', false, text);
                    } catch (execErr) {
                        console.warn('[FlowAutomator] execCommand insertText failed:', execErr);
                    }

                    const browserEditReady = browserEditSucceeded
                        && element.value === text;
                    if (browserEditReady) {
                        const commitActionReady = await waitForRenameCommitAction(700);
                        console.log(
                            '[FlowAutomator] Rename input updated through browser input events.',
                            'Done ready:', commitActionReady
                        );
                        return true;
                    }
                    ...
                    const valueWritten = element.value === text;
                    const commitActionReady = valueWritten
                        && await waitForRenameCommitAction(900);
                    if (!commitActionReady) {
                        console.warn('[FlowAutomator] Flow did not render the rename Done button after editing.');
                    }
                    return valueWritten;
                } else {
        ...
        async function tryRenameFlowAssetOnPage(img, newName, updateStatus) {
            const renameLog = (stage, details = {}) => {
                ...
                try {
                    console.info(`[FlowAutomator][Rename] ${stage} ${JSON.stringify(details)}`);
                } catch (e) {
                    console.info(`[FlowAutomator][Rename] ${stage}`);
                }
                // Keep a page-visible rolling trace so the exact automatic
                // rename failure can be read from DevTools even though this
                // content script runs in an isolated extension world.
                try {
                    const root = document.documentElement;
                    const attribute = 'data-flow-automator-rename-trace';
                    let trace = [];
                    try {
                        trace = JSON.parse(root.getAttribute(attribute) || '[]');
                    } catch (e) { }
                    if (!Array.isArray(trace)) trace = [];
                    trace.push(entry);
                    root.setAttribute(attribute, JSON.stringify(trace.slice(-80)));
                } catch (e) { }
            };
            const describeRenameElement = (element) => {
                if (!element) return null;
                const rect = element.getBoundingClientRect?.();
                return {
                    tag: element.tagName || '',
                    text: getNodeText(element),
                    value: getEditableValue(element),
                    score: element.matches?.('button, [role="button"], [type="submit"]')
                        ? getRenameCommitScore(element)
                        : undefined,
                    disabled: element.matches?.('button, [role="button"], [type="submit"]')
                        ? isDisabledButton(element)
                        : undefined,
                    rect: rect ? {
                        x: Math.round(rect.x),
                        y: Math.round(rect.y),
            ...
            try {
                ...
                // Use the exact console-verified sequence as one main-world
                // operation. Do not split the React hover/menu/input/save state
                // across the isolated content-script world.
                const verifiedMainWorldResult = await tryMainWorldRenameMedia(img, newName);
                renameLog('verified-main-world-result', verifiedMainWorldResult);
                if (verifiedMainWorldResult.responded) {
                    if (typeof updateStatus === 'function') {
                        updateStatus(verifiedMainWorldResult.ok
                            ? `Renamed media on page to "${newName}".`
                            : `Could not rename media on the Flow page to "${newName}" (${verifiedMainWorldResult.step || 'unknown'}: ${verifiedMainWorldResult.error || 'failed'}).`);
                    }
                    if (!verifiedMainWorldResult.ok) {
                        renameLog('failed', {
                            reason: 'verified-main-world-failed',
                            step: verifiedMainWorldResult.step,
                            error: verifiedMainWorldResult.error,
                            actualValue: verifiedMainWorldResult.actualValue
                        });
                    }
                    renameLog('finished', {
                        success: verifiedMainWorldResult.ok,
                        requestedName: newName,
                        method: 'verified-main-world'
                    });
                    return verifiedMainWorldResult.ok;
                }

                renameLog('verified-main-world-unavailable', verifiedMainWorldResult);
                const tile = getTileContainerForMedia(img);
                if (!tile) {
                    renameLog('failed', { reason: 'tile-not-found' });
                    return false;
                }
                const baselineInputs = new Set(getVisibleRenameInputCandidates());
                const baselineButtons = new Set(
                    Array.from(document.querySelectorAll('button, [role="button"], [type="submit"]'))
                        .filter(isVisibleRenameElement)
                );
                renameLog('start', {
                    requestedName: newName,
                    tileId: tile.getAttribute?.('data-tile-id') || tile.querySelector?.('[data-tile-id]')?.getAttribute?.('data-tile-id') || '',
                    mediaHref: img.closest?.('a[href]')?.getAttribute?.('href') || '',
                    baselineInputCount: baselineInputs.size
                });
                ...
                renameLog('menu-open-result', { opened: menuOpened });
                if (!menuOpened) {
                    console.warn('[FlowAutomator] Failed to open overflow menu for renaming.');
                    renameLog('failed', { reason: 'overflow-menu-not-opened' });
                    return false;
                }
                ...
                if (!renameBtn) {
                    ...
                    renameLog('failed', { reason: 'rename-menu-item-not-found' });
                    return false;
                }
                renameLog('rename-menu-item', describeRenameElement(renameBtn));
                
                // Do not focus the Radix menu item before activating it. Flow
                // immediately marks the closed popper aria-hidden; retaining
                // focus inside that hidden ancestor blocks the inline editor's
                // focus and can prevent its Done/Cancel controls from mounting.
                let renameClickedViaBridge = false;
                const closingMenuPopup = renameBtn.closest?.(
                    '[data-radix-popper-content-wrapper], [role="menu"]'
                );
                // Radix focuses its menu content while open. Move focus out
                // before the select handler marks that popper aria-hidden.
                releaseFocusFromAriaHiddenPopup(closingMenuPopup);
                try {
                    renameBtn.click();
                    renameLog('rename-menu-click', { method: 'element.click' });
                } catch (e) {
                    fireMouseClick(renameBtn);
                    renameLog('rename-menu-click', { method: 'pointer-sequence' });
                }
                await new Promise(r => setTimeout(r, 180));
                releaseFocusFromAriaHiddenPopup(closingMenuPopup);
                ...
                for (let attempt = 0; attempt < 10; attempt++) {
                    input = findRenameInputField(img, baselineInputs);
                    if (input) break;
                    await new Promise(r => setTimeout(r, 200));
                }
                // Use the visible textbox as the authoritative result. Retry a
                // full pointer sequence, then the React bridge only as a final
                // menu activation fallback.
                if (!input) {
                    fireMouseClick(renameBtn);
                    await new Promise(r => setTimeout(r, 120));
                    releaseFocusFromAriaHiddenPopup();
                ...
                if (!input && document.contains(renameBtn)) {
                    renameClickedViaBridge = await tryClickElementViaBridge(renameBtn);
                    await new Promise(r => setTimeout(r, 120));
                    releaseFocusFromAriaHiddenPopup();
                    for (let attempt = 0; attempt < 6; attempt++) {
                        input = findRenameInputField(img, baselineInputs);
                ...
                if (!input) {
                    ...
                    renameLog('failed', { reason: 'rename-input-not-found' });
                ...
                renameLog('input-found', describeRenameElement(input));
                releaseFocusFromAriaHiddenPopup();
                // Flow initially renders only the rename textbox. Its Done/Cancel
                // actions are mounted after the textbox itself receives a click.
                // Activate it once before waiting for the commit button.
                try {
                    input.click();
                    renameLog('input-activate', { method: 'element.click' });
                } catch (e) {
                    fireMouseClick(input);
                    renameLog('input-activate', { method: 'pointer-sequence' });
                }
                try { input.focus({ preventScroll: true }); } catch (e) { try { input.focus(); } catch (ignored) {} }

                // The inline image editor mounts its Done/Cancel controls a
                // fraction after that activation click. Wait for the complete editor before
                // changing its controlled value; otherwise Flow may re-render the
                // textbox alone and the action buttons never appear.
                let preWriteSubmitBtn = await waitForCondition(
                    () => findRenameSubmitButton(img, input, baselineInputs, baselineButtons),
                    700,
                    80
                );
                // A click emitted from the isolated content-script world may
                // update the textbox visually without reaching Flow's React
                // onClick handler. The console-proven path runs in the page
                // world, so repeat the activation through the React bridge when
                // the Done control has not mounted yet.
                if (!preWriteSubmitBtn && document.contains(input)) {
                    const activatedViaBridge = await tryClickElementViaBridge(input, { preferClick: true });
                    renameLog('input-activate', {
                        method: 'react-bridge',
                        activated: activatedViaBridge
                    });
                    await new Promise(r => setTimeout(r, 180));
                    releaseFocusFromAriaHiddenPopup();
                    input = findRenameInputField(img, baselineInputs) || input;
                    preWriteSubmitBtn = await waitForCondition(
                        () => findRenameSubmitButton(img, input, baselineInputs, baselineButtons),
                        1800,
                        80
                    );
                }
                renameLog('pre-write-commit-button', {
                    found: !!preWriteSubmitBtn,
                    button: describeRenameElement(preWriteSubmitBtn)
                });
                if (!preWriteSubmitBtn) {
                    console.warn('[FlowAutomator] Rename editor opened without its Done button. Aborting before editing.');
                    renameLog('failed', { reason: 'pre-write-done-button-not-found' });
                ...
                renameLog('input-write-result', {
                    written,
                    input: describeRenameElement(input),
                    commitCandidates: Array.from(document.querySelectorAll('button, [role="button"], [type="submit"]'))
                        .filter(isVisibleRenameElement)
                        .map(describeRenameElement)
                        .filter((candidate) => (candidate?.score || -1) >= 100)
                });
                if (!written) {
                    console.warn('[FlowAutomator] Failed to write new name. Aborting rename to avoid corrupt state.');
                    renameLog('failed', {
                        reason: 'react-name-write-failed',
                        input: describeRenameElement(input)
                    });
                    try { document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true })); } catch (e) {}
                    return false;
                ...
                const findCurrentRenameInput = () => {
                    if (input && document.contains(input) && isVisibleRenameElement(input)) return input;
                    return getVisibleRenameInputCandidates().find((candidate) => {
                        if (baselineInputs.has(candidate)) return false;
                        const value = getEditableValue(candidate).trim();
                        return value === newName.trim()
                            || !!candidate.closest('[role="dialog"], dialog, [role="alertdialog"]');
                    }) || null;
                };
                input = findCurrentRenameInput() || input;
                
                // 5. Find the current submit button after React has processed
                // the input. Do not reuse a dialog reference captured before
                // the controlled input re-render.
                let submitBtn = null;
                for (let attempt = 0; attempt < 10; attempt++) {
                    input = findCurrentRenameInput() || input;
                    submitBtn = findRenameSubmitButton(img, input, baselineInputs, baselineButtons);
                ...
                // Updating a controlled Flow textbox can remount the inline
                // editor without its actions. Reactivate the live textbox once
                // more, exactly as required by the verified console flow, then
                // resolve the newly mounted Done button.
                if ((!submitBtn || isDisabledButton(submitBtn)) && findCurrentRenameInput()) {
                    input = findCurrentRenameInput();
                    try { input.click(); } catch (e) { fireMouseClick(input); }
                    try { input.focus({ preventScroll: true }); } catch (e) { try { input.focus(); } catch (ignored) { } }
                    const reactivatedViaBridge = await tryClickElementViaBridge(input, { preferClick: true });
                    renameLog('post-write-input-activate', {
                        method: 'react-bridge',
                        activated: reactivatedViaBridge,
                        value: getEditableValue(input)
                    });
                    await new Promise(r => setTimeout(r, 220));
                    for (let attempt = 0; attempt < 12; attempt += 1) {
                        input = findCurrentRenameInput() || input;
                        submitBtn = findRenameSubmitButton(img, input, baselineInputs, baselineButtons);
                        if (submitBtn && !isDisabledButton(submitBtn)) break;
                        await new Promise(r => setTimeout(r, 150));
                    }
                }
                renameLog('commit-button-result', {
                    found: !!submitBtn,
                    button: describeRenameElement(submitBtn)
                });
                
                let renameSubmitted = false;
                const renameCommitFinished = (clickedButton) => {
                    if (!findCurrentRenameInput()) return true;
                    return !!clickedButton && (
                        !document.contains(clickedButton)
                        || !isVisibleRenameElement(clickedButton)
                    );
                };
                const submitRenameWithEnter = async () => {
                    const currentInput = findCurrentRenameInput();
                    if (!currentInput) return true;
                    const form = currentInput.closest?.('form');
                    if (form && typeof form.requestSubmit === 'function') {
                        try { form.requestSubmit(); } catch (e) { }
                    }
                    try { currentInput.focus(); } catch (e) {}
                    ...
                    return !!(await waitForCondition(() => !findCurrentRenameInput(), 2500, 120));
                };
                if (submitBtn) {
                    ...
                    if (!isDisabled) {
                        ...
                        // A normal HTMLElement click follows the same React
                        // delegated onClick path as Flow's button. Do this first:
                        // the page-context fiber bridge can acknowledge that it
                        // called a handler even when Flow rejects the fake event.
                        try {
                            submitBtn.click();
                            renameLog('commit-click', { method: 'element.click', button: describeRenameElement(submitBtn) });
                        } catch (clickError) {
                            fireMouseClick(submitBtn);
                            renameLog('commit-click', { method: 'pointer-sequence', button: describeRenameElement(submitBtn) });
                        }
                        await new Promise(r => setTimeout(r, 80));
                        releaseFocusFromAriaHiddenPopup();
                        renameSubmitted = !!(await waitForCondition(
                            () => renameCommitFinished(submitBtn),
                            1400,
                            120
                        ));

                        // Flow can re-render the portal after the input update,
                        // detaching the original Done button. Re-find and click
                        // the live element with the complete pointer sequence.
                        if (!renameSubmitted) {
                            input = findCurrentRenameInput() || input;
                            const liveSubmitBtn = findRenameSubmitButton(
                                img,
                                input,
                                baselineInputs,
                                baselineButtons
                            );
                            if (liveSubmitBtn && !isDisabledButton(liveSubmitBtn)) {
                                fireMouseClick(liveSubmitBtn);
                                renameLog('commit-retry', { method: 'pointer-sequence', button: describeRenameElement(liveSubmitBtn) });
                                await new Promise(r => setTimeout(r, 80));
                                releaseFocusFromAriaHiddenPopup();
                                renameSubmitted = !!(await waitForCondition(
                                    () => renameCommitFinished(liveSubmitBtn),
                                    1400,
                                    120
                                ));

                                // Last click path: call the current React onClick
                                // prop directly, but only after DOM click paths.
                                if (!renameSubmitted && document.contains(liveSubmitBtn)) {
                                    await tryClickElementViaBridge(liveSubmitBtn, { preferClick: true });
                                    renameLog('commit-retry', { method: 'react-bridge', button: describeRenameElement(liveSubmitBtn) });
                                    renameSubmitted = !!(await waitForCondition(
                                        () => renameCommitFinished(liveSubmitBtn),
                                        1600,
                                        120
                        ...
                        if (!renameSubmitted) {
                            renameSubmitted = await submitRenameWithEnter();
                        }
                    }
                } else {
                ...
                // --- SAFETY CLEANUP ---
                // Close only the rename surface. A different Flow dialog may
                // legitimately be open and must not turn a successful rename
                // into a false failure.
                const openRenameInput = findCurrentRenameInput();
                const openRenameSurface = findRenameSurfaceForInput(openRenameInput);
                if (!renameSubmitted && openRenameInput) {
                    ...
                    const stillOpenInput = findCurrentRenameInput();
                    const stillOpenSurface = findRenameSurfaceForInput(stillOpenInput) || openRenameSurface;
                    if (stillOpenInput && stillOpenSurface) {
                        const closeBtns = Array.from(stillOpenSurface.querySelectorAll('button')).filter(isVisibleRenameElement);
                ...
                if (typeof updateStatus === 'function') {
                    updateStatus(renameSubmitted
                        ? `Renamed media on page to "${newName}".`
                        : `Could not rename media on the Flow page to "${newName}".`);
                }
                if (!renameSubmitted) {
                    renameLog('failed', {
                        reason: 'rename-submit-did-not-close-editor',
                        inputStillVisible: !!findCurrentRenameInput(),
                        submitButton: describeRenameElement(submitBtn)
                    });
                }
                renameLog('finished', {
                    success: renameSubmitted,
                    requestedName: newName,
                    inputStillVisible: !!findCurrentRenameInput(),
                    commitStillVisible: !!submitBtn && document.contains(submitBtn) && isVisibleRenameElement(submitBtn)
                });
                return renameSubmitted;
            } catch (err) {
        ...
        const EXPECTED_BRIDGE_VERSION = 50;
        ...
        async function tryMainWorldSetText(targetInput, value) {
            ...
            let targetId = targetInput.getAttribute('data-flow-automator-id');
            if (!targetId) {
                targetId = `set_text_target_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
                targetInput.setAttribute('data-flow-automator-id', targetId);
            }
            ...
            return await new Promise((resolve) => {
                ...
                try {
                    document.dispatchEvent(new CustomEvent('FLOW_AUTOMATOR_SET_TEXT', {
                        detail: { requestId, targetId, value }
                    }));
                } catch (error) {
        ...
        async function tryMainWorldRenameMedia(targetImage, newName) {
            if (!targetImage || !newName) {
                return { responded: true, ok: false, step: 'request', error: 'INVALID_RENAME_REQUEST' };
            }
            const injected = await injectBridgeScript();
            if (!injected) {
                return { responded: false, ok: false, step: 'bridge', error: 'BRIDGE_NOT_LOADED' };
            }

            let targetId = targetImage.getAttribute('data-flow-automator-id');
            if (!targetId) {
                targetId = `rename_media_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
                targetImage.setAttribute('data-flow-automator-id', targetId);
            }
            const requestId = `rename_request_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;

            return await new Promise((resolve) => {
                let settled = false;
                const cleanup = () => {
                    window.removeEventListener('FLOW_AUTOMATOR_RENAME_MEDIA_RESULT', onResult, true);
                };
                const done = (payload) => {
                    if (settled) return;
                    settled = true;
                ...
                const onResult = (evt) => {
                    ...
                    if (detail.requestId !== requestId) return;
                        ...
                        step: detail.step || '',
                        error: detail.error || '',
                        renamedTo: detail.renamedTo || '',
                        actualValue: detail.actualValue ?? null,
                        reactHandlerCalled: detail.reactHandlerCalled === true
                    });
                };
                window.addEventListener('FLOW_AUTOMATOR_RENAME_MEDIA_RESULT', onResult, true);
                try {
                    document.dispatchEvent(new CustomEvent('FLOW_AUTOMATOR_RENAME_MEDIA', {
                        detail: { requestId, targetId, newName }
                    }));
                } catch (error) {
                    done({
                        responded: false,
                        ok: false,
                        step: 'dispatch',
                        error: error?.message || 'RENAME_EVENT_DISPATCH_FAILED'
                    });
                    return;
                }
                setTimeout(() => done({
                    ...
                    ok: false,
                    step: 'timeout',
                    error: 'MAIN_WORLD_RENAME_TIMEOUT'
                }), 15000);
            });
        }
        ...
        const onRuntimeMessage = (message, sender, sendResponse) => {
            if (message.action === CONTENT_ACTIONS.PING || message.action === 'PING') {
            ...
            if (message.action === CONTENT_ACTIONS.PING_VIDEO || message.action === 'PING_VIDEO_V2') {
                if (!hasLikelyPromptComposer()) {
                    sendResponse({ status: 'NOT_READY', error: 'Prompt composer not found' });
                } else {
            ...
            if (message.action === CONTENT_ACTIONS.STOP_PROMPT || message.action === 'STOP_PROMPT_SUBMISSION') {
            ...
            if (message.action === CONTENT_ACTIONS.CLEAR_PROMPT_STOP || message.action === 'CLEAR_PROMPT_SUBMISSION_STOP') {
                if (message.itemId) {
            ...
            if (message.action === CONTENT_ACTIONS.SUBMIT_PROMPT || message.action === 'SUBMIT_PROMPT') {
                if (!hasLikelyPromptComposer()) {
            ...
            if (message.action === CONTENT_ACTIONS.SUBMIT_VIDEO_PROMPT || message.action === 'SUBMIT_VIDEO_PROMPT') {
                if (!hasLikelyPromptComposer()) {
                    sendResponse({ success: false, error: 'Prompt composer not found in this frame.' });
                    return;
                }
            ...
            if (message.action === CONTENT_ACTIONS.VIDEO_DRY_RUN || message.action === 'VIDEO_DRY_RUN') {
                if (!hasLikelyPromptComposer()) {
                    sendResponse({ success: false, error: 'Prompt composer not found in this frame.' });
                    return;
                }
            ...
            if (message.action === CONTENT_ACTIONS.DOWNLOAD_HISTORY || message.action === 'DOWNLOAD_HISTORY') {
                ...
                handleDownloadHistory({ ...message.payload, itemId, isHistoryMode: true })
                    .then(result => {
                        safeSendMessage({ action: 'PROMPT_FINISHED', id: itemId, result });
                    })
                    .catch(err => {
            ...
            if (message.action === CONTENT_ACTIONS.DOWNLOAD_PAGE || message.action === 'DOWNLOAD_PAGE') {
                ...
                handleDownloadHistory({ ...message.payload, itemId, isHistoryMode: false })
                    .then(result => {
                        safeSendMessage({ action: 'PROMPT_FINISHED', id: itemId, result });
                    })
                    .catch(err => {
            ...
            if (message.action === CONTENT_ACTIONS.COLLECT_PAGE_IMAGES || message.action === 'COLLECT_PAGE_IMAGES') {
        ...
        async function handleDownloadHistory({
            ...
            const downloadNameById = new Map(
                (Array.isArray(downloadNameEntries) ? downloadNameEntries : [])
            ...
            const downloadRunId = `${isHistoryMode ? 'history' : 'page'}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
            ...
            try {
                ...
                while (!isHistoryDownloadStopped) {
                    ...
                    for (const nextImage of targetsForThisPass) {
                        ...
                        const fileBaseName = downloadNameById.get(String(itemId)) || '';
                        ...
                        if (wantsUpscaledDownload) {
                            ...
                            const upscaledOk = await directDownloadItem(nextImage, itemId, {
                                ...
                                fileBaseName,
                                downloadRunId,
                                nativeDownloadSource: isHistoryMode ? 'historyDownloader' : 'pageDownloader'
                            }).catch((error) => {
                            ...
                            if (await directDownloadItem(nextImage, itemId, {
                                ...
                                downloadFailures += 1;
                                processedItems.add(itemId); // Continue and report the partial failure at the end
                            }
                        }
                ...
                // Always remove the progress overlay, including partial failures.
                const bar = document.getElementById('flow-progress-bar-container');
                if (bar) bar.remove();

                const missingSelectedCount = selectedIdSet.size
                    ? Math.max(0, selectedIdSet.size - processedItems.size)
                    : 0;
                const failureMessages = [];
                if (upscaleFailures > 0) {
                    failureMessages.push(`Upscale failed for ${upscaleFailures} item(s).`);
                }
                if (downloadFailures > 0) {
                    failureMessages.push(`Download failed for ${downloadFailures} item(s).`);
                }
                if (missingSelectedCount > 0) {
                    failureMessages.push(`${missingSelectedCount} selected item(s) could not be found on the Flow page.`);
                }
                if (failureMessages.length > 0) {
                    return {
                        success: false,
                        count: Math.max(0, processedItems.size - downloadFailures - upscaleFailures),
        ...
        async function handlePromptSubmission({ itemId, quotaAttemptId = '', prompt, settings, selectors }) {
            ...
            console.log(`Starting Automation Flow (v${chrome.runtime.getManifest().version})...`);
                ...
                if (isVideoRun) {
                    ...
                    effectiveSettings.syncFlowSettings === true
                    && (effectiveSettings.flowModel || effectiveSettings.flowAspectRatio || effectiveSettings.flowQuantity)
                ) {
                ...
                const assetsToApply = isVideoRun
                    ? (videoMode === 'ingredients'
                        ? normalizeReferenceAssets(
                            settings?.perPromptAssetsEnabled === true
                                ? (settings?.videoIngredientSelections || [])
                                : ((settings?.videoIngredientSelections || []).length
                                    ? settings.videoIngredientSelections
                                    : (videoQueueEntry ? [videoQueueEntry] : []))
                        )
                        : [])
                ...
                const mentionImageSearchEnabled = isVideoRun
                    ? settings?.perPromptAssetsEnabled === true
                    : (!!settings?.perPromptAssetsEnabled || hasMentionInPrompt);
                ...
                const completionMediaType = completionResult.mediaType
                    || (settings?.flowType === 'video' ? 'video' : 'image');

                if (completionResult.success && settings.autoDownload) {
                    try {
                        if (settings?.flowUpscaledDownload && completionMediaType !== 'video') {
                            ...
                            const upscaledDone = canUpscale
                                ? await directDownloadItem(candidate.el, completionResult.candidateKey || completionResult.url || `generated_${Date.now()}`, {
                                    ...
                                    bulkBaseName: String(settings?.downloadBulkBaseName || '').trim(),
                                    fileBaseName: String(settings?.outputName || '').trim(),
                                    queueItemId: itemId,
                                    nativeDownloadSource: 'generated'
                                })
                                : false;
                            ...
                            if (!upscaledDone) {
                                ...
                                completionResult.upscaledDownloaded = false;
                                completionResult.downloaded = false;
                                completionResult.downloadFailed = true;
                                completionResult.downloadError = `Generated image finished, but the ${upscaleQualityLabel} upscaled download option was not completed.`;
                                // Generation succeeded. Report the download failure
                                // without returning success:false, which would make
                                // the queue regenerate and charge for the prompt again.
                                updateProgress(completionResult.downloadError);
                            } else {
                                completionResult.upscaledDownloaded = true;
                                completionResult.upscaledDownloadQuality = typeof upscaledDone === 'string'
                                    ? upscaledDone
                                    : normalizeUpscaleDownloadQuality(settings?.flowUpscaleQuality);
                                completionResult.downloaded = true;
                                updateProgress(`${completionResult.upscaledDownloadQuality.toUpperCase()} Upscaled download complete. Moving to next prompt...`);
                            }
                            ...
                            const downloadResult = await requestRuntimeMessage({
                                ...
                                queueItemId: itemId,
                                bulkBaseName: String(settings?.downloadBulkBaseName || '').trim(),
                                bulkIndex: (settings.queueIndex ?? 0) + 1,
                                downloadRunId: itemId || `generated_${Date.now()}`,
                                mediaType: completionMediaType
                            });
                            if (!downloadResult?.ok) {
                                completionResult.downloaded = false;
                                completionResult.downloadFailed = true;
                                completionResult.downloadError = downloadResult?.error || 'Download did not start.';
                                updateProgress(`Generated media finished, but download failed: ${completionResult.downloadError}`);
                            } else {
                                completionResult.downloaded = !downloadResult.skipped;
                                completionResult.downloadFilename = downloadResult.filename || '';
                                updateProgress(downloadResult.skipped ? 'Download already handled.' : 'Download sent.');
                            }
                        }
                    } catch (e) {
                        ...
                        completionResult.downloaded = false;
                        completionResult.downloadFailed = true;
                        completionResult.downloadError = e?.message || 'Download failed.';
                        updateProgress(`Generated media finished, but download failed: ${completionResult.downloadError}`);
                    }
                }

                const requestedFlowAssetName = String(settings?.outputName || '').trim();
                if (completionResult.success
                    && completionMediaType !== 'video'
                    && requestedFlowAssetName) {
                    // Flow reports generation complete before the final media
                    // tile and its hover/menu controls have necessarily finished
                    // mounting. Allow that UI to settle, then resolve the live
                    // candidate again instead of retaining a stale image node.
                    updateProgress('Generated image complete. Waiting for Flow media controls before renaming...');
                    await new Promise(r => setTimeout(r, 2500));

                    let renameCandidate = null;
                    let canRenameCandidate = false;
                    for (let attempt = 0; attempt < 6; attempt += 1) {
                        renameCandidate = findCompletionMediaCandidate(serviceSelectors, completionResult);
                        canRenameCandidate = !!(
                            renameCandidate?.el
                            && document.contains(renameCandidate.el)
                            && renameCandidate.el.tagName?.toLowerCase() === 'img'
                        );
                        if (canRenameCandidate) break;
                        await new Promise(r => setTimeout(r, 500));
                    }

                    if (canRenameCandidate) {
                        let renamed = false;
                        for (let renameAttempt = 0; renameAttempt < 3 && !renamed; renameAttempt += 1) {
                            if (renameAttempt > 0) {
                                updateProgress(
                                    `Retrying Flow rename for "${requestedFlowAssetName}" (${renameAttempt + 1}/3)...`
                                );
                                try {
                                ...
                                if (!canRenameCandidate) continue;
                            }

                            renamed = await tryRenameFlowAssetOnPage(
                                renameCandidate.el,
                                requestedFlowAssetName,
                                updateProgress
                            );
                        }
                        completionResult.renamed = renamed;
                        completionResult.renamedTo = renamed ? requestedFlowAssetName : '';
                    } else {
                        completionResult.renamed = false;
                        completionResult.renamedTo = '';
                        updateProgress(`Generated image completed, but Flow rename could not find the new image for "${requestedFlowAssetName}".`);
                    }
                }
