...
import {
    isFirebaseUserDisabledError,
    normalizeFirebaseAuthErrorCode,
    preserveDisabledGateOnMissingRemoteDocument,
    preserveGateAfterAuthRefreshFailure
...
const authAccessDeniedPanel = document.getElementById('authAccessDeniedPanel');
const authAccessDeniedTitle = document.getElementById('authAccessDeniedTitle');
...
const imageDownloaderBulkNameField = document.getElementById('imageDownloaderBulkNameField');
const imageDownloaderBulkNameLabel = document.getElementById('imageDownloaderBulkNameLabel');
const imageDownloaderBulkNameInput = document.getElementById('imageDownloaderBulkNameInput');
const imageDownloaderBulkNameNote = document.getElementById('imageDownloaderBulkNameNote');
const downloadBulkNameLabel = document.getElementById('downloadBulkNameLabel');
const downloadBulkNameInput = document.getElementById('downloadBulkNameInput');
const downloadBulkNameNote = document.getElementById('downloadBulkNameNote');
...
const editModalOutputNameField = document.getElementById('editModalOutputNameField');
const editModalOutputNameLabel = document.getElementById('editModalOutputNameLabel');
const editModalOutputNameInput = document.getElementById('editModalOutputNameInput');
...
let currentEditPromptLocked = false;
...
const uiThemeSimpleOption = document.getElementById('uiThemeSimpleOption');
const uiThemeEinkOption = document.getElementById('uiThemeEinkOption');
...
let pendingCsvFilenameColumnIgnored = false;
let lastUiState = AUTOMATOR_STATE.IDLE;
let lastUiSettings = null;
let lastNonDarkUiTheme = 'eink';
let lastEffectiveUiTheme = null;
let effectiveUiThemeWriteChain = Promise.resolve();
...
const MIN_STARTER_ACCOUNT_PROMPT_LIMIT = 1;
const MAX_STARTER_ACCOUNT_PROMPT_LIMIT = 500;
...
let lastFirebaseTokenRefreshErrorCode = '';
...
async function clearFirebaseAuthState() {
    ...
    lastFirebaseTokenRefreshErrorCode = '';
    devSwitcherAllowedCached = false;
...
async function refreshFirebaseIdTokenIfNeeded({ forceRefresh = false } = {}) {
    if (isSigningOut) return false;
    if (!firebaseAuthState.refreshToken) {
        lastFirebaseTokenRefreshErrorCode = 'MISSING_REFRESH_TOKEN';
        return false;
    }
    if (!forceRefresh && firebaseAuthState.idToken && Date.now() < firebaseAuthState.expiresAt) {
        lastFirebaseTokenRefreshErrorCode = '';
        return true;
    }
    ...
    const activeRateLimitMessage = getActiveAuthRateLimitMessage();
    if (activeRateLimitMessage) {
        lastFirebaseTokenRefreshErrorCode = 'AUTH_RATE_LIMITED';
    ...
    try {
        ...
        lastFirebaseTokenRefreshErrorCode = normalizeFirebaseAuthErrorCode(error?.message) || 'NETWORK_ERROR';
        throw error;
    }
    const json = await res.json().catch(() => ({}));
    if (!res.ok) {
        lastFirebaseTokenRefreshErrorCode = normalizeFirebaseAuthErrorCode(
...
function canEditQueueFileNames(gate = gateState) {
    // Queue-by-queue names start at Starter. The shared Bulk file name remains
    // Premium+, and CSV second-column import remains Professional.
    return canUseBasicImageDownloader(gate);
}
...
function getAccountDisabledMessage() {
    if (['four_or_more_accounts_on_install', 'suspicious_starter_activity', 'firebase_auth_user_disabled'].includes(String(gateState?.disabledReason || ''))) {
        return t('suspiciousAccountDisabled');
    }
    return t('trialExpiredDisabled');
}

function isSuspiciousStarterAccountDisabled(gate = gateState) {
    if (!isAccountDisabled(gate)) return false;
    const reason = String(gate?.disabledReason || '');
    return reason === 'four_or_more_accounts_on_install'
        || reason === 'suspicious_starter_activity'
        || reason === 'firebase_auth_user_disabled'
        || gate?.starterInstallStatus === 'install_account_limit';
}

function formatStarterAccessTimestamp(timestamp) {
    const date = new Date(Number(timestamp) || 0);
    if (!Number.isFinite(date.getTime()) || date.getTime() <= 0) {
        return t('starterMaintenanceScheduledEnd');
    }
    try {
        return new Intl.DateTimeFormat(resolveUiLanguage(currentLanguage), {
            dateStyle: 'medium',
            timeStyle: 'short'
        }).format(date);
    } catch {
...
function getStarterInstallRestrictionMessage(gate = gateState) {
    ...
    if (status === 'starter_maintenance') {
        ...
        const resumeText = endsAt > Date.now()
            ? formatStarterAccessTimestamp(endsAt)
            : t('starterMaintenanceScheduledEnd');
        const remoteReason = String(gate?.starterMaintenanceMessage || '').trim();
        // The legacy admin field contains one untagged string. Show that custom
        // copy only in English; every other saved UI language gets the matching
        // localized maintenance text instead of an unexpected English body.
        const reason = currentLanguage === 'en' && remoteReason
            ? remoteReason
            : t('starterMaintenanceDefault');
        return tFormat('starterMaintenanceBody', { reason, date: resumeText });
    }
    if (status === 'install_account_limit') {
        return t('starterInstallAccountLimit');
    }
    if (status === 'needs_reset') {
        return t('starterInstallNeedsReset');
    }
    const availableAt = Number(gate?.starterInstallResetAvailableAt || 0);
    if (availableAt > Date.now()) {
        return tFormat('starterInstallCooldown', {
            date: formatStarterAccessTimestamp(availableAt)
        });
    }
    return t('starterInstallLocked');
}
...
function applyUpscaleDownloadCopy() {
    ...
    const removeVisibleQuality = (value) => String(value || '')
        .replace(/\s*[:：]\s*(?:2K|4K)\b/gi, '')
        .replace(/\b(?:2K|4K)\b/gi, '')
        .replace(/\s{2,}/g, ' ')
        .trim()
        .replace(/[:：]\s*$/u, '');
    if (labelUpscaleDownload) {
        labelUpscaleDownload.textContent = removeVisibleQuality(t('upscaleDownload'));
    }
    ...
    if (upscaleDownloadQualityPill) {
        ...
        upscaleDownloadQualityPill.textContent = removeVisibleQuality(
            t(isProfessional ? 'upscaleQualityProfessional' : 'upscaleQualityPremium')
        );
...
async function fetchFirestoreGateState({ forceRemote = false } = {}) {
    ...
    let ok = false;
    try {
        // A locally disabled account must prove that Firebase Auth has been
        // re-enabled. Reusing an unexpired ID token could otherwise expose the
        // app during the short interval between a Firestore write and the Auth
        // account update.
        ok = await refreshFirebaseIdTokenIfNeeded({ forceRefresh: gateState.disabled === true });
    } catch (error) {
        console.warn('Firebase token refresh failed; preserving the verified gate:', error?.message || error);
        gateState = preserveGateAfterAuthRefreshFailure(
            gateState,
            firebaseAuthState,
            lastFirebaseTokenRefreshErrorCode
        );
    ...
    if (!ok || !firebaseAuthState.uid || !firebaseAuthState.idToken) {
        ...
        if (isFirebaseUserDisabledError(lastFirebaseTokenRefreshErrorCode)) {
    ...
    if (res.status === 404) {
        const preservedDisabledGate = preserveDisabledGateOnMissingRemoteDocument(gateState);
        if (preservedDisabledGate) {
            gateState = preservedDisabledGate;
    ...
    gateState = {
        ...
        disabledReason: readFirestoreString(fields, 'disabled_reason', ''),
...
async function loadGateStateCache() {
    ...
    if (data[FIRESTORE_GATE_CACHE_KEY]) {
        ...
        const cachedQuotaLimit = Number(gateState.quotaLimit);
        if (Number.isInteger(cachedQuotaLimit)
            && cachedQuotaLimit >= MIN_STARTER_ACCOUNT_PROMPT_LIMIT
            && cachedQuotaLimit <= MAX_STARTER_ACCOUNT_PROMPT_LIMIT) {
...
function applyStarterInstallResult(result = {}) {
    ...
    if (Number.isInteger(reportedQuotaLimit)
        && reportedQuotaLimit >= MIN_STARTER_ACCOUNT_PROMPT_LIMIT
        && reportedQuotaLimit <= MAX_STARTER_ACCOUNT_PROMPT_LIMIT) {
    ...
    const normalized = {
        ...
        ...(result.suspiciousDisabled === true ? {
            disabled: true,
            disabledReason: 'four_or_more_accounts_on_install'
        } : {})
    };
...
function getFriendlyEmailAuthError(message = '') {
    ...
    if (code.includes('USER_DISABLED')) return t('suspiciousAccountDisabled');
...
async function finalizeFirebaseSignInSession() {
    ...
    try {
        ...
        gateState = {
            ...
            disabled: gateState.disabled === true,
            disabledReason: gateState.disabledReason || '',
            specialPermission: false,
            loaded: true,
...
const I18N = {
    en: {
        ...
        suspiciousAccountDisabled: 'Abusive activity has been detected. All linked accounts, including your primary account, have been blocked and can no longer use Google Flow Automator on the Starter plan.',
...
const ACCESS_DENIED_I18N_PATCH = {
    en: {
        accessDeniedKicker: 'STARTER ACCESS',
        accessDeniedTitle: 'Keep your workflow moving',
        accessDeniedUpgradeNote: 'If you would use another account just to keep working, Google Flow Automator is already saving you meaningful time. Upgrade to Premium to continue with this account.',
        hardDisabledKicker: 'ACCESS DISABLED',
        hardDisabledTitle: 'Abusive activity has been detected',
        hardDisabledBody: 'All linked accounts, including your primary account, have been blocked and can no longer use Google Flow Automator on the Starter plan.',
        ...
        starterInstallAccountLimit: 'This browser is already registered to a different Starter (Free) account. Starter allows only one account per browser, so this account cannot access the app. Use the registered account or upgrade this account to Premium.',
        starterInstallNeedsReset: 'The 48-hour security wait is complete. Starter access can now be moved to this browser.',
        starterInstallCooldown: 'This browser is already registered to a different Starter (Free) account. Starter allows only one account per browser, so this account cannot access the app yet. Use the registered account, upgrade this account to Premium, or move Starter access to this browser after {date}.',
        starterInstallLocked: 'This browser is already registered to a different Starter (Free) account, so this account cannot access the app. Use the registered account or upgrade this account to Premium.',
        starterMaintenanceDefault: 'Starter access is temporarily paused due to server maintenance or high demand.',
        starterMaintenanceScheduledEnd: 'the scheduled end time',
        starterMaintenanceBody: '{reason} Starter access will resume automatically at {date}. Premium and Professional memberships are unaffected.'
    },
    ko: {
        accessDeniedKicker: 'STARTER ACCESS',
        accessDeniedTitle: '계정은 하나로, 작업은 계속',
        accessDeniedUpgradeNote: '다른 계정으로라도 계속 사용하고 싶을 만큼 이 앱이 업무를 줄여주고 있다면, 지금 계정을 Premium으로 업그레이드해 작업 흐름을 이어가세요.',
        hardDisabledKicker: '이용 비활성화',
        hardDisabledTitle: '비정상적인 이용 활동이 감지되었습니다',
        hardDisabledBody: '기본 계정을 포함해 이 브라우저에 연결된 모든 계정이 차단되었으며, 더 이상 Starter 플랜으로 Google Flow Automator를 사용할 수 없습니다.',
        ...
        starterInstallAccountLimit: '이 브라우저는 이미 다른 Starter(무료) 계정에 등록되어 있습니다. Starter는 브라우저당 한 계정만 사용할 수 있어 현재 계정으로는 앱에 접속할 수 없습니다. 등록된 계정을 사용하거나 현재 계정을 Premium으로 업그레이드하세요.',
        starterInstallNeedsReset: '48시간 보안 대기가 끝났습니다. 이제 Starter 권한을 이 브라우저로 옮길 수 있습니다.',
        starterInstallCooldown: '이 브라우저는 이미 다른 Starter(무료) 계정에 등록되어 있습니다. Starter는 브라우저당 한 계정만 사용할 수 있어 현재 계정으로는 아직 앱에 접속할 수 없습니다. 등록된 계정을 사용하거나 현재 계정을 Premium으로 업그레이드하세요. 또는 {date} 이후 Starter 권한을 이 브라우저로 옮길 수 있습니다.',
        starterInstallLocked: '이 브라우저는 이미 다른 Starter(무료) 계정에 등록되어 있어 현재 계정으로는 앱에 접속할 수 없습니다. 등록된 계정을 사용하거나 현재 계정을 Premium으로 업그레이드하세요.',
        starterMaintenanceDefault: '서버 점검 또는 높은 이용량으로 Starter 이용이 일시 중지되었습니다.',
        starterMaintenanceScheduledEnd: '예정된 종료 시각',
        starterMaintenanceBody: '{reason} Starter 이용은 {date}에 자동으로 재개됩니다. Premium과 Professional 멤버십은 영향을 받지 않습니다.'
    }
};
...
const COMPLETE_LANGUAGE_COVERAGE_I18N_PATCH = {
    "ko": {
        ...
        "suspiciousAccountDisabled": "비정상적인 이용 활동이 감지되었습니다. 기본 계정을 포함해 이 브라우저에 연결된 모든 계정이 차단되었으며, 더 이상 Starter 플랜으로 Google Flow Automator를 사용할 수 없습니다.",
...
// Keep the dedicated account-access screen fully localized. This patch is
// deliberately applied after the legacy coverage table above so older
// generic "not allowed" copy cannot overwrite the v2.3 access experience.
// The saved UI language is authoritative; applyTranslations() falls back to
// English when that setting is missing or unsupported.
const ACCESS_DENIED_ALL_LANGUAGES_I18N_PATCH = {
    en: {
        suspiciousAccountDisabled: 'Abusive activity has been detected. All linked accounts, including your primary account, have been blocked and can no longer use Google Flow Automator on the Starter plan.',
        accessDeniedKicker: 'STARTER ACCESS',
        accessDeniedTitle: 'Keep your workflow moving',
        ...
        upgradeToPremium: 'Upgrade to Premium',
        useDifferentAccount: 'Use a different account',
        starterInstallAccountLimit: 'This browser is already registered to a different Starter account. Starter allows one account per browser, so use the registered account or upgrade this account to Premium.',
        starterInstallNeedsReset: 'The 48-hour security wait is complete. You can now move Starter access to this browser.',
        starterInstallCooldown: 'This browser is registered to a different Starter account. Use that account, upgrade this account to Premium, or move Starter access here after {date}.',
        starterInstallLocked: 'This browser is registered to a different Starter account. Use the registered account or upgrade this account to Premium.',
        starterMaintenanceDefault: 'Starter access is temporarily paused due to server maintenance or high demand.',
        starterMaintenanceScheduledEnd: 'the scheduled end time',
        starterMaintenanceBody: '{reason} Starter access will resume automatically at {date}. Premium and Professional memberships are unaffected.'
    },
    ko: {
        ...
        upgradeToPremium: 'Premium으로 업그레이드',
        useDifferentAccount: '다른 계정 사용',
        starterInstallAccountLimit: '이 브라우저는 이미 다른 Starter 계정에 등록되어 있습니다. Starter는 브라우저당 한 계정만 사용할 수 있으므로 등록된 계정을 사용하거나 현재 계정을 Premium으로 업그레이드하세요.',
        starterInstallNeedsReset: '48시간 보안 대기가 끝났습니다. 이제 Starter 권한을 이 브라우저로 옮길 수 있습니다.',
        starterInstallCooldown: '이 브라우저는 다른 Starter 계정에 등록되어 있습니다. 등록된 계정을 사용하거나 현재 계정을 Premium으로 업그레이드하거나, {date} 이후 Starter 권한을 이 브라우저로 옮기세요.',
        starterInstallLocked: '이 브라우저는 다른 Starter 계정에 등록되어 있습니다. 등록된 계정을 사용하거나 현재 계정을 Premium으로 업그레이드하세요.',
        starterMaintenanceDefault: '서버 점검 또는 높은 이용량으로 Starter 이용이 일시 중지되었습니다.',
        starterMaintenanceScheduledEnd: '예정된 종료 시각',
        starterMaintenanceBody: '{reason} Starter 이용은 {date}에 자동으로 재개됩니다. Premium 및 Professional 멤버십은 영향을 받지 않습니다.'
    },
    ja: {
        suspiciousAccountDisabled: '不正利用が検出されました。メインアカウントを含む、関連付けられたすべてのアカウントがブロックされ、StarterプランではGoogle Flow Automatorを利用できなくなりました。',
        accessDeniedKicker: 'STARTERアクセス',
        accessDeniedTitle: '1つのアカウントで作業を継続',
        accessDeniedUpgradeNote: '別のアカウントを使ってでも作業を続けたいと思うほど、このアプリがすでに時間と手間を減らしているなら、このアカウントをPremiumにアップグレードして作業を続けてください。',
        hardDisabledKicker: 'アクセスが無効になりました',
        hardDisabledTitle: '不正利用が検出されました',
        hardDisabledBody: 'メインアカウントを含む、関連付けられたすべてのアカウントがブロックされ、StarterプランではGoogle Flow Automatorを利用できなくなりました。',
        upgradeToPremium: 'Premiumにアップグレード',
        useDifferentAccount: '別のアカウントを使用',
        starterInstallAccountLimit: 'このブラウザはすでに別のStarter（無料）アカウントに登録されています。Starterはブラウザごとに1つのアカウントのみ利用できるため、現在のアカウントではアプリにアクセスできません。登録済みのアカウントを使用するか、現在のアカウントをPremiumにアップグレードしてください。',
        starterInstallNeedsReset: '48時間のセキュリティ待機が完了しました。Starterアクセスをこのブラウザに移動できるようになりました。',
        starterInstallCooldown: 'このブラウザはすでに別のStarter（無料）アカウントに登録されています。現在のアカウントではまだアプリにアクセスできません。登録済みのアカウントを使用するか、現在のアカウントをPremiumにアップグレードするか、{date}以降にStarterアクセスをこのブラウザへ移動してください。',
        starterInstallLocked: 'このブラウザはすでに別のStarter（無料）アカウントに登録されているため、現在のアカウントではアプリにアクセスできません。登録済みのアカウントを使用するか、現在のアカウントをPremiumにアップグレードしてください。',
        starterMaintenanceDefault: 'サーバーメンテナンスまたはアクセス集中のため、Starterアクセスは一時停止されています。',
        starterMaintenanceScheduledEnd: '予定されている終了時刻',
        starterMaintenanceBody: '{reason} Starterアクセスは{date}に自動的に再開されます。PremiumおよびProfessionalメンバーシップには影響しません。'
    },
    zh: {
        suspiciousAccountDisabled: '检测到滥用行为。包括主账号在内的所有关联账号均已被封禁，无法再通过Starter方案使用Google Flow Automator。',
        accessDeniedKicker: 'STARTER访问',
        accessDeniedTitle: '一个账号，工作不中断',
        accessDeniedUpgradeNote: '如果您愿意换用其他账号也要继续工作，说明Google Flow Automator已经为您节省了大量时间。请将此账号升级到Premium以继续使用。',
        hardDisabledKicker: '访问已停用',
        hardDisabledTitle: '检测到滥用行为',
        hardDisabledBody: '包括主账号在内的所有关联账号均已被封禁，无法再通过Starter方案使用Google Flow Automator。',
        upgradeToPremium: '升级到Premium',
        useDifferentAccount: '使用其他账号',
        starterInstallAccountLimit: '此浏览器已注册到另一个Starter（免费）账号。Starter每台浏览器仅允许使用一个账号，因此当前账号无法访问此应用。请使用已注册的账号，或将当前账号升级到Premium。',
        starterInstallNeedsReset: '48小时安全等待已结束。现在可以将Starter访问权限转移到此浏览器。',
        starterInstallCooldown: '此浏览器已注册到另一个Starter（免费）账号，因此当前账号暂时无法访问此应用。请使用已注册的账号、将当前账号升级到Premium，或在{date}之后将Starter访问权限转移到此浏览器。',
        starterInstallLocked: '此浏览器已注册到另一个Starter（免费）账号，因此当前账号无法访问此应用。请使用已注册的账号，或将当前账号升级到Premium。',
        starterMaintenanceDefault: '由于服务器维护或访问量过高，Starter访问已暂时暂停。',
        starterMaintenanceScheduledEnd: '预定结束时间',
        starterMaintenanceBody: '{reason} Starter访问将在{date}自动恢复。Premium和Professional会员不受影响。'
    },
    de: {
        suspiciousAccountDisabled: 'Missbräuchliche Nutzung wurde erkannt. Alle verknüpften Konten, einschließlich Ihres Hauptkontos, wurden gesperrt und können Google Flow Automator nicht mehr im Starter-Tarif verwenden.',
        accessDeniedKicker: 'STARTER-ZUGRIFF',
        accessDeniedTitle: 'Ein Konto. Arbeiten Sie weiter.',
        accessDeniedUpgradeNote: 'Wenn Sie sogar ein anderes Konto verwenden würden, um weiterzuarbeiten, spart Ihnen Google Flow Automator bereits wertvolle Zeit. Führen Sie für dieses Konto ein Upgrade auf Premium durch, um fortzufahren.',
        hardDisabledKicker: 'ZUGRIFF DEAKTIVIERT',
        hardDisabledTitle: 'Missbräuchliche Nutzung wurde erkannt',
        hardDisabledBody: 'Alle verknüpften Konten, einschließlich Ihres Hauptkontos, wurden gesperrt und können Google Flow Automator nicht mehr im Starter-Tarif verwenden.',
        upgradeToPremium: 'Auf Premium upgraden',
        useDifferentAccount: 'Anderes Konto verwenden',
        starterInstallAccountLimit: 'Dieser Browser ist bereits für ein anderes Starter-Konto (kostenlos) registriert. Starter erlaubt nur ein Konto pro Browser, daher kann dieses Konto nicht auf die App zugreifen. Verwenden Sie das registrierte Konto oder führen Sie für dieses Konto ein Upgrade auf Premium durch.',
        starterInstallNeedsReset: 'Die 48-stündige Sicherheitswartezeit ist beendet. Der Starter-Zugriff kann jetzt auf diesen Browser übertragen werden.',
        starterInstallCooldown: 'Dieser Browser ist bereits für ein anderes Starter-Konto (kostenlos) registriert. Dieses Konto kann noch nicht auf die App zugreifen. Verwenden Sie das registrierte Konto, führen Sie ein Upgrade auf Premium durch oder übertragen Sie den Starter-Zugriff nach {date} auf diesen Browser.',
        starterInstallLocked: 'Dieser Browser ist bereits für ein anderes Starter-Konto (kostenlos) registriert, daher kann dieses Konto nicht auf die App zugreifen. Verwenden Sie das registrierte Konto oder führen Sie für dieses Konto ein Upgrade auf Premium durch.',
        starterMaintenanceDefault: 'Der Starter-Zugriff ist aufgrund von Serverwartung oder hoher Auslastung vorübergehend pausiert.',
        starterMaintenanceScheduledEnd: 'dem geplanten Endzeitpunkt',
        starterMaintenanceBody: '{reason} Der Starter-Zugriff wird am {date} automatisch fortgesetzt. Premium- und Professional-Mitgliedschaften sind nicht betroffen.'
    },
    fr: {
        suspiciousAccountDisabled: 'Une utilisation abusive a été détectée. Tous les comptes associés, y compris votre compte principal, ont été bloqués et ne peuvent plus utiliser Google Flow Automator avec l’offre Starter.',
        accessDeniedKicker: 'ACCÈS STARTER',
        accessDeniedTitle: 'Un seul compte, un travail sans interruption',
        accessDeniedUpgradeNote: 'Si vous êtes prêt à utiliser un autre compte pour continuer, Google Flow Automator vous fait déjà gagner un temps précieux. Passez ce compte à Premium pour poursuivre votre travail.',
        hardDisabledKicker: 'ACCÈS DÉSACTIVÉ',
        hardDisabledTitle: 'Une utilisation abusive a été détectée',
        hardDisabledBody: 'Tous les comptes associés, y compris votre compte principal, ont été bloqués et ne peuvent plus utiliser Google Flow Automator avec l’offre Starter.',
        upgradeToPremium: 'Passer à Premium',
        useDifferentAccount: 'Utiliser un autre compte',
        starterInstallAccountLimit: 'Ce navigateur est déjà associé à un autre compte Starter (gratuit). Starter n’autorise qu’un compte par navigateur : ce compte ne peut donc pas accéder à l’application. Utilisez le compte enregistré ou passez ce compte à Premium.',
        starterInstallNeedsReset: 'Le délai de sécurité de 48 heures est terminé. L’accès Starter peut maintenant être transféré vers ce navigateur.',
        starterInstallCooldown: 'Ce navigateur est déjà associé à un autre compte Starter (gratuit). Ce compte ne peut pas encore accéder à l’application. Utilisez le compte enregistré, passez ce compte à Premium ou transférez l’accès Starter vers ce navigateur après le {date}.',
        starterInstallLocked: 'Ce navigateur est déjà associé à un autre compte Starter (gratuit) : ce compte ne peut donc pas accéder à l’application. Utilisez le compte enregistré ou passez ce compte à Premium.',
        starterMaintenanceDefault: 'L’accès Starter est temporairement suspendu en raison d’une maintenance du serveur ou d’une forte demande.',
        starterMaintenanceScheduledEnd: 'l’heure de fin prévue',
        starterMaintenanceBody: '{reason} L’accès Starter reprendra automatiquement à {date}. Les abonnements Premium et Professional ne sont pas affectés.'
    },
    hi: {
        suspiciousAccountDisabled: 'दुरुपयोग गतिविधि का पता चला है। आपके मुख्य खाते सहित सभी जुड़े खातों को ब्लॉक कर दिया गया है और अब वे Starter प्लान पर Google Flow Automator का उपयोग नहीं कर सकते।',
        accessDeniedKicker: 'STARTER ऐक्सेस',
        accessDeniedTitle: 'एक खाता, काम जारी रखें',
        accessDeniedUpgradeNote: 'यदि आप काम जारी रखने के लिए दूसरा खाता भी इस्तेमाल करना चाहते हैं, तो Google Flow Automator पहले ही आपका काफी समय बचा रहा है। इसी खाते से जारी रखने के लिए Premium में अपग्रेड करें।',
        hardDisabledKicker: 'ऐक्सेस बंद किया गया',
        hardDisabledTitle: 'दुरुपयोग गतिविधि का पता चला है',
        hardDisabledBody: 'आपके मुख्य खाते सहित सभी जुड़े खातों को ब्लॉक कर दिया गया है और अब वे Starter प्लान पर Google Flow Automator का उपयोग नहीं कर सकते।',
        upgradeToPremium: 'Premium में अपग्रेड करें',
        useDifferentAccount: 'दूसरा खाता इस्तेमाल करें',
        starterInstallAccountLimit: 'यह ब्राउज़र पहले से किसी दूसरे Starter (मुफ़्त) खाते के लिए पंजीकृत है। Starter में प्रति ब्राउज़र केवल एक खाता इस्तेमाल किया जा सकता है, इसलिए यह खाता ऐप नहीं खोल सकता। पंजीकृत खाते का इस्तेमाल करें या इस खाते को Premium में अपग्रेड करें।',
        starterInstallNeedsReset: '48 घंटे की सुरक्षा प्रतीक्षा पूरी हो गई है। अब Starter ऐक्सेस इस ब्राउज़र पर ले जाया जा सकता है।',
        starterInstallCooldown: 'यह ब्राउज़र पहले से किसी दूसरे Starter (मुफ़्त) खाते के लिए पंजीकृत है, इसलिए यह खाता अभी ऐप नहीं खोल सकता। पंजीकृत खाते का इस्तेमाल करें, इस खाते को Premium में अपग्रेड करें, या {date} के बाद Starter ऐक्सेस इस ब्राउज़र पर ले जाएँ।',
        starterInstallLocked: 'यह ब्राउज़र पहले से किसी दूसरे Starter (मुफ़्त) खाते के लिए पंजीकृत है, इसलिए यह खाता ऐप नहीं खोल सकता। पंजीकृत खाते का इस्तेमाल करें या इस खाते को Premium में अपग्रेड करें।',
        starterMaintenanceDefault: 'सर्वर रखरखाव या अधिक मांग के कारण Starter ऐक्सेस अस्थायी रूप से रोका गया है।',
        starterMaintenanceScheduledEnd: 'निर्धारित समाप्ति समय',
        starterMaintenanceBody: '{reason} Starter ऐक्सेस {date} पर अपने आप फिर शुरू होगा। Premium और Professional सदस्यता प्रभावित नहीं हैं।'
    }
};

for (const [lang, patch] of Object.entries(ACCESS_DENIED_ALL_LANGUAGES_I18N_PATCH)) {
    I18N[lang] = { ...(I18N[lang] || I18N.en), ...patch };
}
...
const FILE_NAMING_I18N_PATCH = {
    en: {
        queueFileName: 'File name',
        editQueueItem: 'Edit queue item',
        queueFileNamePlaceholder: 'Download file name (without extension)',
        queueFileNameSaved: 'Queue file name saved.',
        downloadBulkName: 'Bulk file name',
        downloadBulkNamePlaceholder: 'e.g. Picnic (default: GoogleFlowAutomator)',
        downloadBulkNameNote: 'Items without a queue file name download as {name}000, {name}001, …',
        csvFilenameColumnIgnored: 'CSV file-name column was ignored. Bulk CSV file names require Professional.',
        csvGuideMessage: 'CSV Guide\n\nColumn 1: Prompt\nColumn 2: File Name (Professional only)\n\nRecommended header:\nprompt,file name\n\nThe file name is used when the generated image or video is downloaded. Starter and higher can edit each queue file name manually. Professional users can import both columns in bulk. Other memberships import only the prompt column.\n\nUse UTF-8. The selected CSV file is discarded after import.'
    },
    ko: {
        queueFileName: '파일 이름',
        editQueueItem: '큐 항목 수정',
        queueFileNamePlaceholder: '확장자를 제외한 다운로드 파일 이름',
        queueFileNameSaved: '큐 파일 이름을 저장했습니다.',
        downloadBulkName: '벌크 파일 이름',
        downloadBulkNamePlaceholder: '예: Picnic (기본값: GoogleFlowAutomator)',
        downloadBulkNameNote: '큐 파일 이름이 없는 항목은 {name}000, {name}001, … 형식으로 다운로드됩니다.',
        csvFilenameColumnIgnored: 'CSV의 파일 이름 열은 무시했습니다. CSV 벌크 파일 이름은 Professional부터 사용할 수 있습니다.',
        csvGuideMessage: 'CSV 가이드\n\n1열: 프롬프트\n2열: 파일 이름(Professional 전용)\n\n권장 헤더:\nprompt,file name\n\n지정한 파일 이름은 생성 이미지 또는 비디오를 다운로드할 때 사용됩니다. Starter 이상은 큐별 파일 이름을 직접 편집할 수 있고, Professional 사용자는 CSV 두 열을 한 번에 가져올 수 있습니다. 그 외 멤버십은 프롬프트 열만 가져옵니다.\n\nUTF-8을 권장하며 선택한 CSV 파일은 가져오기 후 폐기됩니다.'
    }
};

for (const [lang, patch] of Object.entries(FILE_NAMING_I18N_PATCH)) {
    I18N[lang] = { ...(I18N[lang] || I18N.en), ...patch };
}
...
function normalizeUiTheme(theme = 'eink') {
    if (theme === 'dark') return 'dark';
    if (theme === 'eink' || theme === 'default') return 'eink';
    if (theme === 'simple' || theme === 'logo') return 'simple';
    return 'eink';
}

function resolveStoredUiThemePreference(settings = {}) {
    const rawTheme = settings?.uiTheme;
    let uiTheme = normalizeUiTheme(rawTheme);
    let uiThemeUserSelected = settings?.uiThemeUserSelected === true;

    // Dark and the retired Logo theme could only have come from a deliberate
    // selection in older builds. A marker-less Simple value may instead be an
    // automatic Starter/downgrade fallback from the previous build.
    if (!uiThemeUserSelected && (rawTheme === 'dark' || rawTheme === 'logo')) {
        uiThemeUserSelected = true;
    }
    if (!uiThemeUserSelected && uiTheme === 'simple') {
        uiTheme = 'eink';
    }
    return { uiTheme, uiThemeUserSelected };
}

function hasVerifiedUiThemeEntitlement(gate = gateState) {
    // Explicit gate objects are used by deterministic checks. The live popup
    // state must have been fetched for this exact signed-in account during
    // the current popup session; an old local cache never unlocks a theme.
    if (gate !== gateState) return true;
    return !!firebaseAuthState.uid
        && gateRemoteCheckedThisPopup === true
        && gate?.loaded === true
        && gate?.uid === firebaseAuthState.uid;
}

function isUiThemeMembershipLapsed(gate = gateState) {
    if (gate?.disabled === true || gate?.refunded === true) return true;
    // Active admin/supporter grants remain valid independently of a previous
    // paid subscription, matching the rest of the entitlement system.
    if (gate?.specialPermission === true || hasActiveSupporterEntitlement(gate)) return false;
    const paymentStatus = String(gate?.paymentStatus || '').trim().toLowerCase();
    const membershipStatus = String(gate?.membershipStatus || '').trim().toLowerCase();
    const terminalStatuses = [...LAPSED_PAYMENT_STATUSES, 'expired', 'inactive', 'ended'];
    if (terminalStatuses.includes(paymentStatus)) return true;
    if (terminalStatuses.includes(membershipStatus)) return true;
    return isStripeSubscriptionLapsed(gate);
}

function getAllowedUiThemesForMembership(gate = gateState) {
    // E-Ink is temporarily available to everyone. Starter, signed-out,
    // disabled, refunded, expired and unverified states are fixed to E-Ink
    // and cannot use the theme picker. Dark remains Professional-only.
    if (!hasVerifiedUiThemeEntitlement(gate) || isUiThemeMembershipLapsed(gate)) return ['eink'];
    const tier = getMembershipTier(gate);
    if (tier === 'professional') return ['simple', 'eink', 'dark'];
    if (tier === 'premium' || tier === 'trial') return ['simple', 'eink'];
    return ['eink'];
}

function isUiThemeAllowedForMembership(theme, gate = gateState) {
    return getAllowedUiThemesForMembership(gate).includes(normalizeUiTheme(theme));
}

function getAllowedUiThemeFallback(gate = gateState) {
    const allowedThemes = getAllowedUiThemesForMembership(gate);
    const previousLightTheme = normalizeUiTheme(lastNonDarkUiTheme);
    if (allowedThemes.includes(previousLightTheme)) return previousLightTheme;
    if (allowedThemes.includes('eink')) return 'eink';
    return 'simple';
}

function resolveEffectiveUiTheme(preferredTheme, gate = gateState) {
    const normalizedPreference = normalizeUiTheme(preferredTheme);
    const allowedThemes = getAllowedUiThemesForMembership(gate);
    const preferredThemeAllowed = allowedThemes.includes(normalizedPreference);
    return {
        preferredTheme: normalizedPreference,
        preferredThemeAllowed,
        effectiveTheme: preferredThemeAllowed
            ? normalizedPreference
            : getAllowedUiThemeFallback(gate)
    };
}

function syncEffectiveUiThemeSetting(theme) {
    const effectiveTheme = normalizeUiTheme(theme);
    if (lastEffectiveUiTheme === effectiveTheme) return effectiveUiThemeWriteChain;
    lastEffectiveUiTheme = effectiveTheme;
    effectiveUiThemeWriteChain = effectiveUiThemeWriteChain
        .then(() => chrome.storage.local.set({
            [STORAGE_KEYS.UI_THEME_EFFECTIVE]: effectiveTheme
        }))
        .catch((error) => {
            console.warn('Failed to sync effective UI theme:', error);
        });
    return effectiveUiThemeWriteChain;
}

function applyTheme(theme = 'eink', { rememberLightTheme = true } = {}) {
    const normalized = normalizeUiTheme(theme);
    // `theme-logo` was an unreleased legacy value. Remove it explicitly so
    // only the three supported themes can ever be active.
    document.documentElement.classList.remove('theme-logo');
    document.documentElement.classList.toggle('theme-simple', normalized === 'simple');
    document.documentElement.classList.toggle('theme-dark', normalized === 'dark');
    document.documentElement.dataset.theme = normalized;
    if (normalized !== 'dark' && rememberLightTheme) lastNonDarkUiTheme = normalized;
    syncDarkModeToggleIcon(normalized);
}

function syncDarkModeToggleIcon(theme = 'simple') {
    if (!darkModeToggleBtn) return;
    const isDark = normalizeUiTheme(theme) === 'dark';
    const label = isDark ? 'Switch to Light Mode' : 'Switch to Dark Mode';
    darkModeToggleBtn.textContent = isDark ? '☀' : '◐';
    darkModeToggleBtn.title = label;
    darkModeToggleBtn.setAttribute('aria-label', label);
}
...
function resolveUiLanguage(lang) {
    ...
    return Object.hasOwn(I18N, normalized) ? normalized : 'en';
}

function applyTranslations(lang) {
    currentLanguage = resolveUiLanguage(lang);
    ...
    setText('authAccessDeniedKicker', t('accessDeniedKicker'));
    setText('authAccessDeniedTitle', t('accessDeniedTitle'));
    setText('authAccessDeniedUpgradeNote', t('accessDeniedUpgradeNote'));
    ...
    setText('imageDownloaderBulkNameLabel', t('downloadBulkName'));
    if (imageDownloaderBulkNameInput) imageDownloaderBulkNameInput.placeholder = t('downloadBulkNamePlaceholder');
    setText('imageDownloaderBulkNameNote', tFormat('downloadBulkNameNote', {
        name: sanitizeOutputFileName(imageDownloaderBulkNameInput?.value || '') || 'GoogleFlowAutomator'
    }));
    setText('downloadBulkNameLabel', t('downloadBulkName'));
    if (downloadBulkNameInput) downloadBulkNameInput.placeholder = t('downloadBulkNamePlaceholder');
    setText('downloadBulkNameNote', tFormat('downloadBulkNameNote', {
        name: sanitizeOutputFileName(downloadBulkNameInput?.value || '') || 'GoogleFlowAutomator'
    }));
    ...
    // Theme product names stay consistent in every UI language.
    setText('uiThemeSimpleOption', 'Simple');
    setText('uiThemeEinkOption', 'E-Ink');
    setText('uiThemeDarkOption', 'Dark');
    ...
    setText('editModalOutputNameLabel', t('queueFileName'));
    if (editModalOutputNameInput) editModalOutputNameInput.placeholder = t('queueFileNamePlaceholder');
...
function syncAuthLandingVisibility() {
    ...
    const installRestricted = !!firebaseAuthState.uid && isStarterInstallRestricted();
    const suspiciousDisabled = isSuspiciousStarterAccountDisabled();
    const hardBlocked = suspiciousDisabled
        || (installRestricted && gateState.starterInstallStatus === 'install_account_limit');
    const accessDenied = installRestricted || suspiciousDisabled;
    const genericDisabled = disabled && !suspiciousDisabled;
    const signedIn = isSignedInForFeatures();
    const showAuthLanding = accessDenied || (!signedIn && !disabled);
    document.body.classList.toggle('signed-out-auth-view', showAuthLanding);
    document.body.classList.toggle('account-disabled-view', genericDisabled);
    ...
    authAccessDeniedPanel?.classList.toggle('hidden', !accessDenied);
    ...
    if (accessDenied) {
        ...
        setText('authAccessDeniedKicker', t(hardBlocked ? 'hardDisabledKicker' : 'accessDeniedKicker'));
        setText('authAccessDeniedTitle', t(hardBlocked ? 'hardDisabledTitle' : 'accessDeniedTitle'));
        setText('authAccessDeniedBody', hardBlocked
            ? t('hardDisabledBody')
            : getStarterInstallRestrictionMessage());
        setText('authAccessDeniedUpgradeNote', t('accessDeniedUpgradeNote'));
    }
    const needsHostPermission = installRestricted
        && !hardBlocked
        && gateState.starterInstallStatus === 'host_permission_required';
    const canMoveStarterHere = installRestricted
        && !hardBlocked
        && gateState.starterInstallStatus === 'needs_reset';
...
function applyPremiumFeatureLocks() {
    ...
    const allowedUiThemes = getAllowedUiThemesForMembership();
    ...
    // Theme ladder (temporary): everyone receives E-Ink, Starter is fixed to
    // it with no picker access, Premium may choose Simple/E-Ink, and only
    // Professional may choose Dark. Saved paid preferences are retained.
    if (uiThemeSimpleOption) uiThemeSimpleOption.disabled = !allowedUiThemes.includes('simple');
    if (uiThemeEinkOption) uiThemeEinkOption.disabled = !allowedUiThemes.includes('eink');
    if (uiThemeDarkOption) uiThemeDarkOption.disabled = !allowedUiThemes.includes('dark');
    if (uiThemeSelect) uiThemeSelect.disabled = allowedUiThemes.length === 1;

    const {
        preferredThemeAllowed,
        effectiveTheme
    } = resolveEffectiveUiTheme(lastUiSettings?.uiTheme || 'eink');
    applyTheme(effectiveTheme, { rememberLightTheme: preferredThemeAllowed });
    if (uiThemeSelect) uiThemeSelect.value = effectiveTheme;
    void syncEffectiveUiThemeSetting(effectiveTheme);

    // The header shortcut stays visible, but the click handler applies the
    // same entitlement check as the dropdown.
    if (darkModeToggleBtn) {
        if (allowedUiThemes.includes('dark')) {
            syncDarkModeToggleIcon(effectiveTheme);
        } else {
            const lockedLabel = t('darkModeProfessionalOnly');
            darkModeToggleBtn.title = lockedLabel;
            darkModeToggleBtn.setAttribute('aria-label', lockedLabel);
        }
    }
...
function applySubscriptionGate() {
    ...
    const bulkNameUnlocked = canUsePremiumOnlyTools();
    ...
    if (imageDownloaderBulkNameField) {
        imageDownloaderBulkNameField.classList.toggle('hidden', !bulkNameUnlocked);
    }
    if (imageDownloaderBulkNameInput) {
        imageDownloaderBulkNameInput.disabled = !bulkNameUnlocked;
        imageDownloaderBulkNameInput.title = bulkNameUnlocked ? '' : t('premiumFeatureLocked');
    }
...
async function syncSettingWithPage(setting, value, label) {
    // Direct manipulation of Flow's composer options is Professional-only.
    // Keep Premium and Starter accounts on the settings already chosen in Flow.
    if (!hasProfessionalTierAccess()) return false;
...
async function init() {
    ...
    // Normalize legacy values and distinguish a deliberate selection from an
    // automatic fallback: `default` becomes E-Ink and `logo` becomes Simple.
    const {
        uiTheme: normalizedInitialTheme,
        uiThemeUserSelected: initialThemeWasUserSelected
    } = resolveStoredUiThemePreference(settings);
    if (settings && (
        settings.uiTheme !== normalizedInitialTheme
        || settings.uiThemeUserSelected !== initialThemeWasUserSelected
        || Object.prototype.hasOwnProperty.call(settings, 'uiThemeEffective')
    )) {
        settings = {
            ...settings,
            uiTheme: normalizedInitialTheme,
        ...
        // v2.3 moves the entitlement-filtered value to its own key. Remove
        // the legacy nested field so it cannot race with preference writes.
        delete settings.uiThemeEffective;
        await chrome.storage.local.set({ [STORAGE_KEYS.SETTINGS]: settings });
    }

    applyTranslations(settings?.uiLanguage || 'en');
    if (uiLanguageSelect) uiLanguageSelect.value = currentLanguage;
    // E-Ink is the public baseline. updateUI applies a saved Premium or
    // Professional choice only after the remote entitlement is verified.
    applyTheme('eink', { rememberLightTheme: false });
    if (uiThemeSelect) uiThemeSelect.value = 'eink';
    ...
    setInterval(() => {
        if (document.hidden || !firebaseAuthState.uid) return;
        if (isAccountDisabled()) {
            // A paid upgrade may re-enable this UID while the panel remains
            // open. Recheck Auth and Firestore without allowing automation.
            void refreshGateState({ forceRemote: true }).then(() => {
                applySubscriptionGate();
                void refreshUI();
                updateProfileModalUi();
            });
    ...
    if (saveEditBtn) {
        saveEditBtn.addEventListener('click', async () => {
            ...
            if (currentlyEditingId && (newPrompt || currentEditPromptLocked)) {
                ...
                if (idx !== -1) {
                    const nextPrompt = currentEditPromptLocked ? queue[idx].prompt : newPrompt;
                    let updatedItem = { ...queue[idx], prompt: nextPrompt };
                    if (canEditQueueFileNames()) {
                        updatedItem.outputName = sanitizeOutputFileName(editModalOutputNameInput?.value || '');
                    }
                    if (!currentEditPromptLocked) {
    ...
    startBtn.addEventListener('click', async () => {
        ...
        if (state === AUTOMATOR_STATE.RUNNING) {
            ...
            // A new run starts with a clean result history. Keep only work that
            // is still pending/in progress; completed and failed items remain
            // reusable through their explicit Reuse controls until Run is pressed.
            let finalQueue = await storage.getQueue();
            const cleanedQueue = finalQueue.filter((item) =>
                item.status !== QUEUE_STATUS.COMPLETED
                && item.status !== QUEUE_STATUS.FAILED
            );
            if (cleanedQueue.length !== finalQueue.length) {
                await storage.setQueue(cleanedQueue);
                finalQueue = cleanedQueue;
            }
            ...
            if (settings.flowType === 'video') {
                ...
                const usePerPromptVideoAssets = settings.perPromptAssetsEnabled === true;
                const globalVideoAssets = sanitizeVideoAssetQueue(settings.videoAssetQueue || [], settings.videoAvailableAssets || []);
                for (const item of runnableVideoItems) {
                    const sanitized = sanitizeVideoSettings({
                        ...
                        videoEndFrameSelection: usePerPromptVideoAssets
                            ? (item.videoEndFrameSelection || null)
                            : (settings.videoEndFrameSelection || null),
                    ...
                    if (mode === FLOW_VIDEO_MODES.FRAMES) {
                        const globalAssetIndex = item.videoOriginalIndex ?? finalQueue.findIndex((entry) => entry.id === item.id);
                        const globalStartFrame = settings.videoStartFrameSelection
                            || settings.videoStartImage
                            || globalVideoAssets[globalAssetIndex]
                            || globalVideoAssets[0]
                            || null;
                        const hasStartFrame = usePerPromptVideoAssets
    ...
    getDownloadBulkNameInputs().forEach((input) => {
        input.addEventListener('input', () => syncDownloadBulkNameUi({
            ...(lastUiSettings || {}),
            downloadBulkBaseName: input.value
        }, input));
        input.addEventListener('change', () => {
            saveDownloadBulkBaseName(input).catch((error) => {
                showGateStatus(error?.message || String(error), true);
            });
        });
    });
        ...
        if (message.action === 'PROMPT_FINISHED' && (message.id?.startsWith('history_download_') || message.id?.startsWith('page_download_'))) {
            ...
            if (statusBar) {
                if (message.result?.success === false) {
                    const errorMessage = message.result?.error || 'Download failed.';
                    statusBar.classList.remove('hidden');
    ...
    if (uiThemeSelect) {
        uiThemeSelect.addEventListener('change', async () => {
            let theme = normalizeUiTheme(uiThemeSelect.value);
            if (!isUiThemeAllowedForMembership(theme)) {
                showGateStatus(theme === 'dark' ? t('darkModeProfessionalOnly') : t('premiumFeatureLocked'), true);
                theme = getAllowedUiThemeFallback();
                uiThemeSelect.value = theme;
                applyTheme(theme, { rememberLightTheme: false });
                void syncEffectiveUiThemeSetting(theme);
                return;
            }
            applyTheme(theme);
            await storage.updateSettings({
            ...
            await syncEffectiveUiThemeSetting(theme);
            refreshUI();
        });
    }
    if (darkModeToggleBtn) {
        darkModeToggleBtn.addEventListener('click', async () => {
            if (!isUiThemeAllowedForMembership('dark')) {
                showGateStatus(t('darkModeProfessionalOnly'), true);
                return;
            }
            const current = normalizeUiTheme(lastUiSettings?.uiTheme || 'eink');
            const next = current === 'dark' ? normalizeUiTheme(lastNonDarkUiTheme) : 'dark';
            applyTheme(next);
            ...
            await syncEffectiveUiThemeSetting(next);
        });
    }
...
function getDownloadBulkNameInputs() {
    return [imageDownloaderBulkNameInput, downloadBulkNameInput].filter(Boolean);
}

function syncDownloadBulkNameUi(settings = lastUiSettings || {}, sourceInput = null) {
    const inputs = getDownloadBulkNameInputs();
    if (!inputs.length) return;
    const explicitSource = inputs.includes(sourceInput) ? sourceInput : null;
    const focusedSource = inputs.find((input) => document.activeElement === input) || null;
    const draftSource = explicitSource || focusedSource;
    const savedValue = sanitizeOutputFileName(settings.downloadBulkBaseName || '');
    const displayValue = draftSource ? draftSource.value : savedValue;

    inputs.forEach((input) => {
        if (input !== draftSource) input.value = displayValue;
    });

    const previewName = sanitizeOutputFileName(displayValue) || 'GoogleFlowAutomator';
    [imageDownloaderBulkNameNote, downloadBulkNameNote].forEach((note) => {
        if (note) note.textContent = tFormat('downloadBulkNameNote', { name: previewName });
    });
}

async function saveDownloadBulkBaseName(sourceInput = downloadBulkNameInput || imageDownloaderBulkNameInput) {
    const inputs = getDownloadBulkNameInputs();
    if (!sourceInput || !inputs.includes(sourceInput) || !canUsePremiumOnlyTools()) return;
    const downloadBulkBaseName = sanitizeOutputFileName(sourceInput.value);
    inputs.forEach((input) => {
        input.value = downloadBulkBaseName;
    });

    const previousSettings = lastUiSettings;
    lastUiSettings = { ...(lastUiSettings || {}), downloadBulkBaseName };
    syncDownloadBulkNameUi(lastUiSettings, sourceInput);
    try {
        await storage.updateSettings({ downloadBulkBaseName });
    } catch (error) {
        lastUiSettings = previousSettings;
        syncDownloadBulkNameUi(lastUiSettings || {});
        throw error;
    }
}
...
function renderDownloadPicker() {
    ...
    if (downloadBulkNameInput) {
        downloadBulkNameInput.disabled = !premiumUnlocked;
    }
...
async function startPageImageDownload({ selectedIds = null } = {}) {
    ...
    const bulkBaseName = sanitizeOutputFileName(downloadBulkNameInput?.value || '');
    await storage.updateSettings({ downloadBulkBaseName: bulkBaseName });
    lastUiSettings = { ...(lastUiSettings || {}), downloadBulkBaseName: bulkBaseName };
    syncDownloadBulkNameUi(lastUiSettings);
    const selectedIdSet = Array.isArray(selectedIds) && selectedIds.length
        ? new Set(selectedIds)
        : null;
    const assetsToDownload = downloadPickerAssets.filter((asset) => (
        asset?.id && (!selectedIdSet || selectedIdSet.has(asset.id))
    ));
    const queue = await storage.getQueue();
    const normalizeDownloadPrompt = (value = '') => String(value || '')
        .normalize('NFC')
        .replace(/^\[Model:\s*[^\]]+\]\s*/i, '')
        .replace(/[\s.…]*(?:\.\.\.|…)$/, '')
        .replace(/\s+/g, ' ')
        .trim()
        .toLowerCase();
    const resolveQueueFileName = (prompt = '') => {
        const target = normalizeDownloadPrompt(prompt);
        if (!target) return '';
        const matched = queue.find((item) => {
            const candidate = normalizeDownloadPrompt(item?.prompt || '');
            if (!candidate) return false;
            const matchLength = Math.min(80, target.length, candidate.length);
            if (matchLength < 12) return target === candidate;
            return candidate.includes(target.slice(0, matchLength))
                || target.includes(candidate.slice(0, matchLength));
        });
        return sanitizeOutputFileName(matched?.outputName || '');
    };
    const fallbackBaseName = bulkBaseName || 'GoogleFlowAutomator';
    // Resolve the final base name per scanned card before the crawler starts.
    // This avoids losing the chosen name when Flow virtualizes/reorders cards.
    const downloadNameEntries = assetsToDownload.map((asset, index) => ({
        id: String(asset.id),
        name: resolveQueueFileName(asset.prompt)
            || `${fallbackBaseName}${String(index).padStart(3, '0')}`
    }));
    const payload = {
        tabId: tab.id,
        bulkBaseName,
...
async function saveSettings() {
    ...
    const bulkNameInputValue = imageDownloaderBulkNameInput?.value
        ?? downloadBulkNameInput?.value
        ?? '';
    const allowedDownloadBulkBaseName = canUsePremiumOnlyTools()
        ? sanitizeOutputFileName(bulkNameInputValue)
        : '';
    const updatedSettings = {
        ...
        downloadBulkBaseName: allowedDownloadBulkBaseName,
...
async function openStoryboardOverview() {
    ...
    // The standalone Storyboard consumes the derived theme so a saved Dark
    // preference cannot leak through after a downgrade, while a verified paid
    // user's E-Ink/Simple/Dark choice appears immediately on first paint.
    await syncEffectiveUiThemeSetting(document.documentElement.dataset.theme);
...
function updateUI(queue, state, settings, logs) {
    ...
    if (settings) {
        ...
        syncDownloadBulkNameUi(settings);
        syncUpscaleDownloadAvailability();
...
function sanitizeOutputFileName(value = '') {
    ...
    cleaned = Array.from(cleaned).slice(0, 80).join('').replace(/[ .]+$/g, '');
    return cleaned;
}

function openQueueItemEditModal(item, originalIndex, settings = {}, { focusName = false } = {}) {
    if (!item || !editModal || !editModalInput) return;
    const queueNamingUnlocked = canEditQueueFileNames();
    const defaultName = settings.flowType === 'video'
        ? `Video ${originalIndex + 1}`
        : `Image ${originalIndex + 1}`;
    currentlyEditingId = item.id;
    currentEditModalMode = 'single';
    currentEditPromptLocked = item.status === QUEUE_STATUS.COMPLETED || item.status === QUEUE_STATUS.FAILED;
    editModalInput.value = item.prompt || '';
    editModalInput.placeholder = '';
    editModalInput.disabled = currentEditPromptLocked;
    if (labelEditPrompt) {
        labelEditPrompt.textContent = queueNamingUnlocked ? t('editQueueItem') : t('editPrompt');
    }
    if (editModalOutputNameField) {
        editModalOutputNameField.classList.toggle('hidden', !queueNamingUnlocked);
    }
    if (editModalOutputNameInput) {
        editModalOutputNameInput.value = sanitizeOutputFileName(item.outputName || '');
        editModalOutputNameInput.placeholder = defaultName;
        editModalOutputNameInput.disabled = item.status === QUEUE_STATUS.IN_PROGRESS;
    }
    editModal.classList.remove('hidden');
    if (focusName && queueNamingUnlocked && !editModalOutputNameInput?.disabled) {
        editModalOutputNameInput.focus();
        editModalOutputNameInput.select();
    } else if (!currentEditPromptLocked) {
        editModalInput.focus();
    } else if (queueNamingUnlocked && !editModalOutputNameInput?.disabled) {
        editModalOutputNameInput.focus();
    }
}
...
function renderQueue(queue, settings = {}) {
    ...
    if (queue.length === 0) {
        ...
        const buildQueueItemEl = (item, originalIndex) => {
            ...
            const defaultQueueName = settings.flowType === 'video'
                ? `Video ${originalIndex + 1}`
                : `Image ${originalIndex + 1}`;
            const fullQueueName = sanitizeOutputFileName(item.outputName || '') || defaultQueueName;
            const queueNameChars = Array.from(fullQueueName);
            textSpan.textContent = queueNameChars.length > 10
                ? `${queueNameChars.slice(0, 10).join('')}...`
                : fullQueueName;
            textSpan.title = fullQueueName;
            if (canEditQueueFileNames() && item.status !== QUEUE_STATUS.IN_PROGRESS) {
                textSpan.classList.add('editable-name');
                textSpan.tabIndex = 0;
                textSpan.setAttribute('role', 'button');
                textSpan.setAttribute('aria-label', `${fullQueueName}. ${t('queueFileNamePlaceholder')}`);
                const openNameEditor = (event) => {
                    event.stopPropagation();
                    openQueueItemEditModal(item, originalIndex, settings, { focusName: true });
                };
                textSpan.addEventListener('click', openNameEditor);
                textSpan.addEventListener('keydown', (event) => {
                    if (event.key === 'Enter' || event.key === ' ') {
                        event.preventDefault();
                        openNameEditor(event);
                    }
                });
            }
            ...
            if (isCompletedItem || isFailedItem) {
                ...
                editBtn.title = canEditQueueFileNames() ? t('editQueueItem') : t('editPrompt');
                editBtn.innerHTML = '✎';
                editBtn.onclick = (e) => {
                    e.stopPropagation();
                    openQueueItemEditModal(item, originalIndex, settings);
                };
            }
            ...
            if (settings.flowType === 'video') {
                ...
                // Asset tiles belong to the per-prompt asset mode. With the
                // toggle off, the top-level Video Assets list is the source of
                // truth and queue items must not expose their own asset slots.
                if (showPerPromptAssets) {
                ...
                if (showPerPromptAssets || perPromptModelEnabled || uniqueWarnings.length) {
...
async function handleAddPrompts(options = {}) {
    ...
    const newItems = sequences.map((seq, idx) => {
        ...
        const csvNames = pendingCsvOutputNames.get(seq.trim()) || [];
        const csvName = hasProfessionalTierAccess()
            ? sanitizeOutputFileName(csvNames.shift() || '')
            : '';
...
async function reuseCompletedQueueItem(itemId, options = {}) {
    ...
    const metadata = {};
    metadata.outputName = sanitizeOutputFileName(sourceItem.outputName || '');
...
function detectPromptColumn(rows = []) {
    ...
    const firstRow = rows[0].map((v) => String(v || '').trim().toLowerCase().replace(/[_-]+/g, ' '));
    ...
    const nameKeywords = [
        'name', 'file name', 'filename', 'output name', 'save as',
        '이름', '파일 이름', '파일명', '출력 이름', '名前'
    ];
    const promptCol = firstRow.findIndex((cell) => headerKeywords.some((keyword) => cell === keyword || cell.includes(keyword)));
    const nameCol = firstRow.findIndex((cell) => nameKeywords.some((kw) => (
        cell === kw || (kw !== 'name' && kw !== '이름' && cell.includes(kw))
    )));
    if (promptCol !== -1) return { promptCol, nameCol, startRow: 1, headerDetected: true };

    // Headerless two-column files follow the documented bulk format:
    // first column = prompt, second column = file name.
    const maxCols = rows.reduce((max, row) => Math.max(max, Array.isArray(row) ? row.length : 0), 0);
    if (maxCols >= 2) {
        return { promptCol: 0, nameCol: 1, startRow: 0, headerDetected: false };
    }

    // Single-column language-agnostic fallback.
    let bestCol = 0;
    let bestNonEmpty = -1;
    let bestLength = -1;
...
function buildCsvPromptEntries(rows = [], { allowOutputNames = false } = {}) {
    ...
    for (let rowIndex = startRow; rowIndex < rows.length; rowIndex++) {
        ...
        const outputName = allowOutputNames && nameCol !== -1
            ? sanitizeOutputFileName(cells[nameCol] || '')
            : '';
...
function cancelCsvValidationModal() {
    pendingCsvEntries = [];
    pendingCsvFilenameColumnIgnored = false;
...
function applyCsvEntriesToPromptInput(entries = []) {
    ...
    if (hasProfessionalTierAccess()) {
        validEntries.forEach((e) => {
            const outputName = sanitizeOutputFileName(e.outputName || '');
            if (!outputName) return;
            const names = pendingCsvOutputNames.get(e.prompt) || [];
            names.push(outputName);
            pendingCsvOutputNames.set(e.prompt, names);
        });
    }
...
function applyCsvValidationFixes() {
    ...
    showGateStatus(
        pendingCsvFilenameColumnIgnored
            ? t('csvFilenameColumnIgnored')
            : tFormat('csvLoadedReady', { count: importedCount, details: detailSuffix }),
        pendingCsvFilenameColumnIgnored
    );
...
async function handleCsvImport(e) {
    ...
    try {
        ...
        const csvLayout = detectPromptColumn(rows);
        const allowOutputNames = hasProfessionalTierAccess();
        pendingCsvFilenameColumnIgnored = csvLayout.nameCol !== -1 && !allowOutputNames;
        const entries = buildCsvPromptEntries(rows, { allowOutputNames });
        if (!entries.length) {
        ...
        if (pendingCsvFilenameColumnIgnored) {
            showGateStatus(t('csvFilenameColumnIgnored'), true);
        } else if (decoded.hadReplacement) {
            showGateStatus(
...
function resetEditModalState() {
    ...
    currentEditModalMode = 'single';
    currentEditPromptLocked = false;
    if (editModalInput) editModalInput.disabled = false;
    if (editModalOutputNameInput) {
        editModalOutputNameInput.value = '';
        editModalOutputNameInput.disabled = false;
    }
    editModalOutputNameField?.classList.add('hidden');
    if (labelEditPrompt) {
...
async function runVideoDryRun() {
    ...
    const payload = {
        action: 'GFA_V222_14_VIDEO_DRY_RUN',
        payload: {
