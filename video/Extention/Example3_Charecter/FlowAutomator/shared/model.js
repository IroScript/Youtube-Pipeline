...
export const STORAGE_KEYS = {
    ...
    // Derived from the saved preference plus the currently verified tier.
    // Keep it outside SETTINGS so entitlement refreshes cannot overwrite a
    // user's theme preference through a competing settings read/write.
    UI_THEME_EFFECTIVE: 'flow_ui_theme_effective',
...
export const DEFAULT_SETTINGS = {
    ...
    autoDownload: true,
    // Premium+ global filename prefix shared by generated auto-download and
    // the Image Downloader picker. Empty means use the built-in fallback.
    downloadBulkBaseName: '',
    ...
    concurrentCount: 1,
    uiLanguage: 'en',
    // E-Ink is the temporary baseline for every tier. Starter is fixed to it;
    // Premium and Professional may save other themes allowed by their tier.
    uiTheme: 'eink',
    uiThemeUserSelected: false,
...
export function autoBindAssetsByPromptMentions(item, settings) {
    ...
    if (isVideo) {
        ...
        // With per-prompt assets disabled, the global Video Assets queue is
        // authoritative. Do not silently create hidden scene-level overrides
        // from @mentions; voice parsing remains independent.
        if (!settings.perPromptAssetsEnabled) {
            return voiceMatch
                ? { ...item, videoVoiceReference: extractedVoice }
                : item;
        }
