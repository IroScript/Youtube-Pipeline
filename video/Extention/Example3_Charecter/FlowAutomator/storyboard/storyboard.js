...
function normalizeUiTheme(theme = 'eink') {
    ...
    if (normalized === 'simple' || normalized === 'logo') return 'simple';
    if (normalized === 'dark') return 'dark';
    if (normalized === 'eink' || normalized === 'default') return 'eink';
    return 'eink';
}

function applyUiTheme(theme = 'eink') {
    const normalized = normalizeUiTheme(theme);
    const root = document.documentElement;
    root.classList.remove('theme-logo');
    root.classList.toggle('theme-simple', normalized === 'simple');
...
function createAvailableAssetStrip(settings = {}) {
    ...
    const canAssignPerPrompt = settings.perPromptAssetsEnabled === true;
...
function renderScene(item, index, settings) {
    ...
    // The storyboard only exposes scene-level asset editing when explicitly
    // enabled. Otherwise assets are global and remain in the top asset strip.
    const showImageAssets = !!settings.perPromptAssetsEnabled;
    if (showImageAssets) {
...
function renderMeta(items, settings) {
    ...
    const chips = [
        ...
        settings.perPromptAssetsEnabled === true ? t('dragHint') : ''
    ];
...
async function render() {
    const [queue, settings, effectiveThemeData] = await Promise.all([
        storage.getQueue(),
        storage.getSettings(),
        chrome.storage.local.get(STORAGE_KEYS.UI_THEME_EFFECTIVE)
    ]);
    // Standalone pages consume only the entitlement-filtered value. E-Ink is
    // the public baseline when the popup has not written a current value yet.
    applyUiTheme(effectiveThemeData?.[STORAGE_KEYS.UI_THEME_EFFECTIVE] || 'eink');
    ...
    const perPromptHelp = settings.perPromptAssetsEnabled === true
        ? `
...
chrome.storage.onChanged.addListener((changes, area) => {
    if (area === 'local') {
        ...
        if (changes[STORAGE_KEYS.UI_THEME_EFFECTIVE]) {
            shouldRefresh = true;
        }
